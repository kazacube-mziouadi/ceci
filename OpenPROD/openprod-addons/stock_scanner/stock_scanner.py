...
class scanner_scenario(models.Model):
    ...
    company_id = fields.Many2one('res.company', string='Company', required=True, default=lambda self: self.env.user.company_id, ondelete='restrict', help='Company to be used on this scenario')
    hardware_ids = fields.Many2many('scanner.hardware', 'scanner_hardware_scenario_rel', 'scenario_id', 'hardware_id', string='Allowed hardwares')
    ack_code = fields.Char(string='Acknowledgement code', size=32, required=True, default='ACK', help="Acknowledgement code in steps")
...
class scanner_scenario_step(models.Model):
    ...
    def save_position(self):
        ''' Évite une erreur si on clique sur save position dans l'éditeur de scenario '''
        pass
...
class scanner_hardware(models.Model):
    ...
    scenario_ids = fields.Many2many('scanner.scenario', 'scanner_hardware_scenario_rel', 'hardware_id', 'scenario_id', string='Allowed scenariis', domain=[('active', '=', True)])
...
#             # Evaluate the condition for each transition
    ...
    def _scenario_list(self, parent_id=False):
        ...
        scanner_scenario_obj = self.env['scanner.scenario']
        scanner_scenario_ids = scanner_scenario_obj.search([('warehouse_ids', 'in', [self.warehouse_id.id]), ('parent_id', '=', parent_id), ('hardware_ids', 'in', self.ids)])

        return_value = [data.name for data in scanner_scenario_ids if data.name]
        if not return_value:
            return_value = [_('No scenario available'), _('Please contact'), _('your administrator')]
