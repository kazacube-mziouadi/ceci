...
class mrp_manufacturingorder(models.Model):
    ...
    # COLUMNS
    #===========================================================================
    simulation_id = fields.Many2one('mrp.simulation', string='simulation', required=False, ondelete='restrict', select=True)


    def plannification_mo_done(self, is_sublevel=False, automatic_purchase=False):
        """ 
            Si l'OF terminé est l'Of de tête d'une simulation, celle ci est passée à l'état terminée
        """
        res = super(mrp_manufacturingorder, self).plannification_mo_done(is_sublevel=is_sublevel, automatic_purchase=automatic_purchase)
        simulation_rcs = self.env['mrp.simulation'].search([('mo_id', 'in', self.ids)])
        if simulation_rcs:
            simulation_rcs.wkf_done()
...
class mrp_workorder(models.Model):
    ...
    @api.one
    @api.depends('mo_id.simulation_id')
    def _compute_simulation_id(self):
        """
            Fonction qui récupère la simulation de l'OF
        """
        self.simulation_id = self.mo_id.simulation_id.id
    
    
    #===========================================================================
    # COLUMNS
    #===========================================================================
    simulation_id = fields.Many2one('mrp.simulation', string='simulation', compute='_compute_simulation_id', store=True, select=True)
    ...
    def plannification_mo_at_earlier(self, date, mo, is_sublevel=False, is_procur_level_manufact=False,
        ...
        res = super(mrp_workorder, self).plannification_mo_at_earlier(
            date, 
            mo, 
        ...
        if mo.simulation_id and not first_mo_id:
            wo_rs = self.search([('simulation_id', '=', mo.simulation_id.id)])
            if wo_rs:
    ...
    def plannification_mo_at_the_latest(self, date, mo, is_sublevel=False, is_procur_level_manufact=False,
        ...
        res = super(mrp_workorder, self).plannification_mo_at_the_latest(
            date, 
            mo, 
            is_sublevel=is_sublevel, 
            is_procur_level_manufact=is_procur_level_manufact, 
        ...
        if mo.simulation_id and first_pass:
            wo_rs = self.search([('simulation_id', '=', mo.simulation_id.id)])
            if wo_rs:
                wo_rs.compute_sorting_level()
