...
class DecimalPrecisionFloat(orm.AbstractModel):
    ...
    def precision(self, cr, uid, column, options=None, context=None):
        ...
        if dp:
            return self.pool['decimal.precision'].precision_get(cr, uid, dp)

        return super(DecimalPrecisionFloat, self).precision(cr, uid, column, options=options, context=context)