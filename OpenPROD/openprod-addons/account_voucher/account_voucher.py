...
class account_move(osv.osv):
    ...
    _columns = {
        'voucher_id':fields.many2one('account.voucher', 'Voucher', copy=False, select=True),
    }
    
    
    def additional_function_domain(self, arg):
        """
            Fonction qui permet de faire un point d'entrée pour modifier le domain du search dans un module qui hérite de account_move
        """
        arg0, arg1, arg_1 = False, False, False
        if arg[0] == 'domain_search_move_invoice':
            arg0 = 'id'
            arg1 = 'in'
            name = arg[-1][0]
            credit = arg[-1][1]
            debit = arg[-1][2]
            partner_id = arg[-1][3]
            payment_move_ids = arg[-1][4] and arg[-1][4][0] and arg[-1][4][0][-1] or []
            amount_deviation = arg[-1][5]
            line_id = arg[-1][-1]
            move_invoice_ids = self.env['account.bank.statement.openprod'].search_move_invoice(name, credit, debit, partner_id, amount_deviation, self.env['account.invoice'], payment_move_ids=payment_move_ids, line_id=line_id)
            if move_invoice_ids:
                arg_1 = move_invoice_ids
            else:
                arg_1 = []
                
        elif arg[0] == 'domain_search_move_voucher':
            arg0 = 'id'
            arg1 = 'in'
            journal_id = arg[-1][0]
            credit = arg[-1][1]
            ...
            currency_id = arg[-1][5]
            date =arg[-1][6]
            amount_deviation = arg[-1][7]
            nb_day_deviation = arg[-1][8]
            abso_currency_id = arg[-1][9]
            line_id = arg[-1][-1]
            move_voucher_ids = self.env['account.bank.statement.openprod'].search_move_payment(journal_id, credit, debit, partner_id, account_id, currency_id, date, 
                                                                                               amount_deviation, nb_day_deviation, abso_currency_id, self.env['account.move.line'], line_id=line_id)
            if move_voucher_ids:
                arg_1 = move_voucher_ids
            else:
                arg_1 = []
    ...
    def compute_domain_args_move(self, args):
    ...
    def search(self,args=None, offset=0, limit=None, order=None, count=None):
        args = args or []
        args_modified = self.compute_domain_args_move(args)
        return super(account_move,self).search(args=args_modified, offset=offset, limit=limit, order=order, count=count) 
    ...
    def name_search(self, name, args=None, operator='ilike', limit=1000):
        ...
        args = self.compute_domain_args_move(args)
...
class account_voucher(osv.osv):
    ...
    def _compute_move_ids(self, cr, uid, ids, name, args, context=None):
        res = {}
        for voucher in self.browse(cr, uid, ids, context=context):
            res[voucher.id] = {}
            audit = False
            ...
            for move in voucher.account_move_ids:
                if move.name == voucher.number and move.to_check:
                    audit = True
                
                move_lines.extend([line.id for line in move.line_id])
            
            res[voucher.id]['audit'] = audit
            res[voucher.id]['move_ids'] = move_lines
            
        return res
    
    def _set_check_value(self, cr, uid, ids, field_name, field_value, args, context=None):
        move_obj = self.pool.get('account.move')
        for voucher in self.browse(cr, uid, ids, context=context):
            move_id = move_obj.search(cr, uid, [('voucher_id', '=', voucher.id), ('name', '=', voucher.number)], limit=1, context=context)
            if move_id:
                move_obj.write(cr, uid, move_id, {'to_check': True}, context)
    ...
    def _compute_writeoff_amount(self, cr, uid, line_dr_ids, line_cr_ids, amount, type):
        debit = credit = 0.0
        debit_line_amount = credit_line_amount = 0.0
        writeoff_amount = 0
        invoice_writeoff_amount = 0
        sign = type == 'payment' and -1 or 1
        for l in line_dr_ids:
            if isinstance(l, dict):
                debit += l['amount']
                if l['amount']:
                    debit_line_amount += l['amount_unreconciled']
                
        for l in line_cr_ids:
            if isinstance(l, dict):
                credit += l['amount']
                if l['amount']:
                    credit_line_amount += l['amount_unreconciled']
        
        if abs(credit_line_amount - debit_line_amount) > amount:
            invoice_writeoff_amount = abs(credit_line_amount - debit_line_amount) - sign * (credit - debit)
        else:
            writeoff_amount = amount - sign * (credit - debit)
        
        return writeoff_amount, invoice_writeoff_amount

    def onchange_line_ids(self, cr, uid, ids, line_dr_ids, line_cr_ids, amount, voucher_currency, type, context=None):
        context = context or {}
        if not line_dr_ids and not line_cr_ids:
            return {'value':{'writeoff_amount': 0.0, 'invoice_writeoff_amount': 0.0}}
        # resolve lists of commands into lists of dicts
        line_dr_ids = self.resolve_2many_commands(cr, uid, 'line_dr_ids', line_dr_ids, ['amount', 'amount_unreconciled'], context)
        line_cr_ids = self.resolve_2many_commands(cr, uid, 'line_cr_ids', line_cr_ids, ['amount', 'amount_unreconciled'], context)
        ...
        writeoff_amount, invoice_writeoff_amount = self._compute_writeoff_amount(cr, uid, line_dr_ids, line_cr_ids, amount, type)
        return {'value': {'writeoff_amount': writeoff_amount, 'invoice_writeoff_amount': invoice_writeoff_amount,'is_multi_currency': is_multi_currency}}
    ...
    def _get_writeoff_amount(self, cr, uid, ids, name, args, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            if abs(credit_line_amount - debit_line_amount) > voucher.amount:
                invoice_writeoff_amount = currency_obj.round(cr, uid, currency, abs(credit_line_amount - debit_line_amount) - sign * (credit - debit))
            else:
                writeoff_amount = currency_obj.round(cr, uid, currency, voucher.amount - sign * (credit - debit))
            
            res[voucher.id]['invoice_writeoff_amount'] = invoice_writeoff_amount
...
#    _rec_name = 'number'
    ...
    _columns = {
        ...
        'account_partner_id':fields.many2one('account.account', 'Account partner', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        ...
        'account_move_ids':fields.one2many('account.move', 'voucher_id', 'Account moves', readonly=True, copy=False),
        'move_ids': fields.function(_compute_move_ids, type='one2many', relation='account.move.line', string='Journal Items', multi='move_ids'),
        'audit': fields.function(_compute_move_ids, type='boolean', fnct_inv=_set_check_value, multi='move_ids', string='To Review',
        ...
        'writeoff_amount': fields.function(_get_writeoff_amount, string='Difference Amount', type='float', readonly=True, help="Computed as the difference between the amount stated in the voucher and the sum of allocation on the voucher lines.", multi='writeoff_amount'),
        'invoice_writeoff_amount': fields.function(_get_writeoff_amount, string='Invoice difference Amount', type='float', readonly=True, help="Computed as the difference between the invoice amount and the voucher amount", multi='writeoff_amount'),
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None, context=None, count=None):
        for arg in args:
            if arg[0] == 'domain_voucher_no_used_id':
                index = args.index(arg)
                del args[index]
                arg[0] = 'id'
                arg[-1] = arg[-1] and [x[1] for x in arg[-1]] or []    
                args.insert(index, tuple(arg))
                
        return super(account_voucher,self).search(cr, uid, args, offset=offset, limit=limit, order=order, context=context, count=count)
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=80):
        ...
        return super(account_voucher, self).name_search(cr, uid, name, args=args, operator=operator, context=context, limit=limit)
    ...
    def compute_tax(self, cr, uid, ids, context=None):
        ...
        for voucher in voucher_pool.browse(cr, uid, ids, context=context):
            ...
            taxes = position_pool.map_tax(cr, uid, partner and partner.property_account_position_id or False, tax, context=context)
            ...
            if not tax[0].price_include:
                for line in voucher.line_ids:
                    for tax_line in tax_pool.compute_all(cr, uid, tax, line.amount, False, 1).get('taxes', []):
                ...
                for line in voucher.line_ids:
                    ...
                    for tax_line in tax_pool.compute_all(cr, uid, tax, line.untax_amount or line.amount, False, 1).get('taxes', []):
    ...
    def onchange_price(self, cr, uid, ids, line_ids, tax_id, partner_id=False, context=None):
        ...
        for line in line_ids:
            ...
            if tax_id:
                ...
                if not tax[0].price_include:
                    for tax_line in tax_pool.compute_all(cr, uid, tax, line_amount, False, 1).get('taxes', []):
    ...
    def onchange_journal_voucher(self, cr, uid, ids, line_ids=False, tax_id=False, price=0.0, partner_id=False, journal_id=False, ttype=False, company_id=False, context=None):
        ...
        if ttype in ('sale', 'receipt'):
            account_partner_id = partner and partner.property_account_receivable_id and partner.property_account_receivable_id.id or False
        else:
            account_partner_id = partner and partner.property_account_payable_id and partner.property_account_payable_id.id or False
            
        if journal.type in ('sale','sale_refund'):
            account_id = partner and partner.property_account_receivable_id and partner.property_account_receivable_id.id or False
            account_partner_id = journal.default_debit_account_id and journal.default_debit_account_id.id or False
            tr_type = 'sale'
        elif journal.type in ('purchase', 'purchase_refund','expense'):
            account_id = partner and partner.property_account_payable_id and partner.property_account_payable_id.id or False
            account_partner_id = journal.default_credit_account_id and journal.default_credit_account_id.id or False
            tr_type = 'purchase'
        else:
        ...
        default['value']['account_partner_id'] = account_partner_id
    ...
    def basic_onchange_partner(self, cr, uid, ids, partner_id, journal_id, ttype, context=None):
        ...
        journal = journal_pool.browse(cr, uid, journal_id, context=context)
        partner = partner_id and partner_pool.browse(cr, uid, partner_id, context=context)
        account_id = False
        if journal.type in ('sale','sale_refund'):
            account_id = partner and partner.property_account_receivable_id.id or False
        elif journal.type in ('purchase', 'purchase_refund','expense'):
            account_id = partner and partner.property_account_payable_id.id or False
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=None):
        ...
        partner = partner_id and self.pool.get('res.partner').browse(cr, uid, partner_id, context=context) or False
        if ttype in ('sale', 'receipt'):
            account_partner_id = partner and partner.property_account_receivable_id and partner.property_account_receivable_id.id or False
        else:
            account_partner_id = partner and partner.property_account_payable_id and partner.property_account_payable_id.id or False
        
        res['value']['account_partner_id'] = account_partner_id
        return res

    def recompute_voucher_lines(self, cr, uid, ids, partner_ids, journal_id, price, currency_id, ttype, date, context=None):
        """
        Returns a dict that contains new values and context

        @param partner_ids: value from field partner_id and his invoiced partners
        @param args: other arguments
        @param context: context arguments, like lang, time zone
        ...
        if not partner_ids or not journal_id:
            return default
        ...
        if not context.get('move_line_ids', False):
            ids = move_line_pool.search(cr, uid, [('state','=','valid'), ('account_id.type', '=', account_type), ('reconcile_id', '=', False), ('partner_id', 'in', partner_ids)], context=context)
        else:
        ...
        for line in account_move_lines:
            ...
            if invoice_id:
                if line.invoice.id == invoice_id or line.invoice.id in invoice_ids:
        ...
        for line in account_move_lines:
            ...
            writeoff_amount, invoice_writeoff_amount = self._compute_writeoff_amount(cr, uid, default['value']['line_dr_ids'], default['value']['line_cr_ids'], price, ttype)
            default['value']['writeoff_amount'] = writeoff_amount
            default['value']['invoice_writeoff_amount'] = invoice_writeoff_amount
    ...
    def action_cancel_draft(self, cr, uid, ids, context=None):
        self.reset_workflow(cr, uid, ids)
        return True
    ...
    def cancel_voucher(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            voucher.refresh()
            for account_move in voucher.account_move_ids:
...
#                 voucher_partner = voucher.partner_id
#                 #Si le partenaire est un contact lié à une société, on récupère le
#                 #compte de la société. Sinon on récupère son compte
#                 if not voucher.partner_id.is_company:
#                     if voucher.partner_id.parent_id:
#                         voucher_partner = voucher.partner_id.parent_id
#                     else:
#                         voucher_partner = voucher.partner_id
#                         
#                 if voucher.type in ('sale', 'receipt'):
#                     account_id = voucher_partner.property_account_receivable_id.id
#                 else:
#                     account_id = voucher_partner.property_account_payable_id.id
            else:
                # fallback on account of voucher
                account_id = voucher.account_partner_id.id
            sign = voucher.type == 'payment' and -1 or 1
            move_line = {
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        move_pool = self.pool.get('account.move')
        move_line_pool = self.pool.get('account.move.line')
        rec_writeoff_pool = self.pool.get('account.move.line.reconcile.writeoff')
        for voucher in self.browse(cr, uid, ids, context=context):
            debit_line_amount = credit_line_amount = 0.0
            amount_to_writeoff = 0
            invoiced_amount = 0
            local_context = dict(context, force_company=voucher.journal_id.company_id.id)
            for account_move in voucher.account_move_ids:
                if voucher.number == account_move.name:
                    continue
            
            for debit_line in voucher.line_dr_ids:
                debit_line_amount += debit_line.amount_unreconciled
            for credit_line in voucher.line_cr_ids:
                credit_line_amount += credit_line.amount_unreconciled
            if abs(credit_line_amount - debit_line_amount) > voucher.amount:
                invoiced_amount = abs(credit_line_amount - debit_line_amount)
            ...
            move_pool.write(cr, uid, [move_id], {'voucher_id': voucher.id}, context=context)
            ...
            if invoiced_amount and line_total < invoiced_amount:
                amount_to_writeoff = invoiced_amount - line_total
            ...
            #Possibilité de lettrer avec ajustement pour un montant inférieur
            if amount_to_writeoff and voucher.payment_option == 'with_writeoff':
                today = time.strftime('%Y-%m-%d')
                ctx['data_read'] = {'writeoff_acc_id': voucher.writeoff_acc_id.id,
                                        'date_p': today,
                                        'journal_id': voucher.journal_id.id,
                                        'comment': voucher.comment,
                                        'analytic_id': voucher.analytic_id and voucher.analytic_id.id or False,
                                        }
            else: 
            ...
            if amount_to_writeoff and voucher.payment_option == 'with_writeoff':
                if rec_list_ids:
                    ctx['active_ids'] = rec_list_ids[0]
                    rec_writeoff_pool.trans_rec_reconcile(cr, uid, [], context=ctx)
            else:
                reconcile = False
