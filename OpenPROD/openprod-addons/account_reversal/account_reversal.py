...
class account_move(models.Model):
    ...
    @api.multi
    def _move_reversal(self, reversal_date, reversal_period_id=False, reversal_journal_id=False, move_prefix=False, move_line_prefix=False, reconcile=False):
        """
        ...
        if reversal_period_id:
            ...
            reversal_period = period_obj.with_context(company_id=self.company_id.id, account_period_prefer_normal=True).find(reversal_date)[0]
        ...
        for rev_move_line in reversal_move.line_id:
            ...
            if reconcile and rev_move_line.account_id.reconcile:
                rec_dict.setdefault((rev_move_line.account_id, rev_move_line.partner_id), amlo.browse(False))
                rec_dict[(rev_move_line.account_id, rev_move_line.partner_id)] += rev_move_line
