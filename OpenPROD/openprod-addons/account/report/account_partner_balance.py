...
class partner_balance(report_sxw.rml_parse, common_report_header):
    ...
    def lines(self):
        ...
        full_account = []
        self.cr.execute(
            "SELECT l.account_id,ac.name AS account_name,ac.code AS code,p.name, sum(debit) AS debit, sum(credit) AS credit, " \
            ...
            "AND am.state IN %s " \
            "AND " + self.query + "" \
            "GROUP BY p.id, p.name,l.account_id,ac.name,ac.code " \
