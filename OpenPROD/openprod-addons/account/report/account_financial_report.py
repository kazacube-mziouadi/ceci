    ...
    def get_lines(self, data):
        ...
        for report in self.pool.get('account.financial.report').browse(self.cr, self.uid, ids2, context=data['form']['used_context']):
            ...
            if report.type == 'accounts' and report.account_ids:
                account_ids = account_obj._get_children(self.cr, self.uid, [x.id for x in report.account_ids])
            elif report.type == 'account_type' and report.account_type_ids:
                report_type_ids = [x.id for x in report.account_type_ids]
                account_ids = account_obj.search(self.cr, self.uid, ['|', ('debit_account_type_id','in', report_type_ids),
                                                                     ('credit_account_type_id','in', report_type_ids)])
            if account_ids:
