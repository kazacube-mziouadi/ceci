...
class account_move_line(osv.osv):
    ...
    def _query_get(self, cr, uid, obj='l', context=None):
        ...
        if context.get('chart_account_id'):
            child_ids = account_obj._get_children(cr, uid, [context['chart_account_id']], context=context)
    ...
    def _default_get(self, cr, uid, fields, context=None):
        ...
        if context.get('journal_id'):
            ...
            if isinstance(data.get('partner_id'), (int, long)):
                ...
                part = False
            if account and part:
                account = fiscal_pos_obj.map_account(cr, uid, part and part.property_account_position_id or False, account.id)
    ...
    _columns = {
        ...
        'ref': fields.related('move_id', 'ref', string='Reference', type='char', store = {
    ...
    def onchange_partner_id(self, cr, uid, ids, move_id, partner_id, account_id=None, debit=0, credit=0, date=False, journal=False, context=None):
        ...
        if jt and jt in ('purchase', 'purchase_refund') and part.property_supplier_payment_term_id:
            payment_term_id = part.property_supplier_payment_term_id.id
        elif jt and part.property_payment_term_id:
            payment_term_id = part.property_payment_term_id.id
        if payment_term_id:
            res = payment_term_obj.compute(cr, uid, payment_term_id, 100, date)
            if res:
                val['date_maturity'] = isinstance(res[0][0], str) and res[0][0] or res[0][0][0] 
                
        if not account_id:
            id1 = part.property_account_payable_id.id
            id2 =  part.property_account_receivable_id.id
            if jt:
                if jt in ('sale', 'purchase_refund'):
                    val['account_id'] = fiscal_pos_obj.map_account(cr, uid, part and part.property_account_position_id or False, id2)
                elif jt in ('purchase', 'sale_refund'):
                    val['account_id'] = fiscal_pos_obj.map_account(cr, uid, part and part.property_account_position_id or False, id1)
    ...
    def onchange_account_id(self, cr, uid, ids, account_id=False, partner_id=False, context=None):
        ...
        if account_id:
            ...
            if tax_ids and partner_id:
                part = partner_obj.browse(cr, uid, partner_id, context=context)
                tax_id = fiscal_pos_obj.map_tax(cr, uid, part and part.property_account_position_id or False, tax_ids, context=context)[0]
            else:
    ...
    def create(self, cr, uid, vals, context=None, check=True):
        ...
        if ('account_id' in vals):
            ...
            if journal.type_control_ids:
                type_id = False
                if account.credit_account_type_id:
                    type_id = account.credit_account_type_id.id
                elif account.debit_account_type_id:
                    type_id = account.debit_account_type_id.id
        ...
        if vals.get('account_tax_id', False):
            ...
            base_adjusted = False
            for tax in tax_obj.compute_all(cr, uid, [tax_id], total, False,1.00, force_excluded=False).get('taxes'):
