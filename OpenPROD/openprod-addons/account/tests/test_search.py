...
class TestSearch(TransactionCase):
    ...
    def test_property_unset_search(self):
        ...
        partner_ids = self.res_partner_model.search(cr, uid, [('property_payment_term_id', '=', False), ('id', '=', self.a_partner)])
        self.assertTrue(partner_ids, "unset property field 'propety_payment_term' should have been found")

        self.res_partner_model.write(cr, uid, [self.a_partner], {'property_payment_term_id': self.a_payment_term})
        partner_ids = self.res_partner_model.search(cr, uid, [('property_payment_term_id', '=', False), ('id', '=', self.a_partner)])
