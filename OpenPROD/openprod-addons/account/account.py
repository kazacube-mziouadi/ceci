...
class account_payment_term_line(osv.osv):
    ...
    _columns = {
        ...
        'days': fields.integer('Number of Days', required=True, help="Number of days to to the computation"),
...
class account_account_type(osv.osv):
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('account.account.type', 'Parent section', ondelete='cascade', domain=[('type','=','aggregation')]),
        'type': fields.selection([('aggregation', 'Aggregation'),
                                  ('normal', 'Normal')], 'Type', required=False),
        'profit_loss_group': fields.selection([('no_group', 'No group'),
                                               ('group_1', 'Group 1'),
                                               ('group_2', 'Group 2'),
                                               ('group_3', 'Group 3'),
                                               ('group_4', 'Group 4'),
                                               ...
                                               ('group_9', 'Group 9'),
                                               ('group_10', 'Group 10'),
                                               ], 'Group', required=False, help="Select the group for compute report total"),
        'report_type': fields.function(_get_current_report_type, fnct_inv=_save_report_type, type='selection', string='Report type', store=True,
            selection= [('none','/'),
        ...
        'report_level': fields.selection([('first', 'First'),
                                          ('second', 'Second'),
                                          ('third', 'Third'),
                                          ('fourth', 'Fourth')], 'Report level', required=False),
        'bold_report': fields.boolean('Bold in report'),
        'report_comment': fields.text("Comment in page's bottom of the report"),
        'debit_account_ids': fields.one2many('account.account', 'debit_account_type_id', 'Account list using this section for debit'),
        'credit_account_ids': fields.one2many('account.account', 'credit_account_type_id', 'Account list using this section for credit'),
    }
    _defaults = {
        'report_type': 'none',
        'bold_report': False,
...
class account_account(osv.osv):
    ...
    def _where_calc(self, cr, uid, domain, active_test=True, context=None):
        ...
        while pos < len(domain):
            ...
            if domain[pos][0] == 'journal_id':
                ...
                ids1 = super(account_account, self).search(cr, uid, ['|', ('debit_account_type_id', 'in', ids3), 
                                                                          ('credit_account_type_id', 'in', ids3)])
    ...
    def _get_children(self, cr, uid, ids, context=None):
        #this function search for all the children of the given account ids
    ...
    def __compute(self, cr, uid, ids, field_names, arg=None, context=None,
        ...
        if children:
            ...
            # children, in that case join on a
            # values() e.g.:
            # SELECT l.account_id as id FROM account_move_line l
            # INNER JOIN (VALUES (id1), (id2), (id3), ...) AS tmp (id)
            # ON l.account_id = tmp.id
            # or make _get_children return a query and join on that
...
#                if can_compute:
    ...
    _columns = {
        ...
        'debit_account_type_id': fields.many2one('account.account.type', 'Report section if debit', required=False, domain=[('type','=','normal')]),
        'credit_account_type_id': fields.many2one('account.account.type', 'Report section if credit', required=False, domain=[('type','=','normal')]),
        ...
        'level': fields.function(_get_level, string='Level', method=True, type='integer',
             store={
                    'account.account': (_get_children, ['level', 'parent_id'], 10),
                   }),
 ...
 'Unreconciled' will copy only the journal items that were unreconciled on the first day of the new fiscal year."""),
    ...
    def _check_account_type(self, cr, uid, ids, context=None):
        for account in self.browse(cr, uid, ids, context=context):
            if account.type in ('receivable', 'payable') and account.close_method != 'unreconciled':
                return False
    ...
    _constraints = [
        ...
        (_check_account_type, 'Configuration Error!\nYou cannot select an account type with a deferral method different of "Unreconciled" for accounts with internal type "Payable/Receivable".', ['debit_account_type_id', 'credit_account_type_id','type']),
    ...
    _sql_constraints = [
        ...
        ('view_type_check', "CHECK((type='view' AND debit_account_type_id IS NULL AND credit_account_type_id IS NULL AND close_method='none') OR "
        "(type!='view'))", "A view type account cannot have debit or credit report section and it's close method must be 'None'"),
    ]
    def name_search(self, cr, user, name, args=None, operator='ilike', context=None, limit=100):
        ...
        try:
            if name and str(name).startswith('partner:'):
                ...
                args += [('id', 'in', (part.property_account_payable_id.id, part.property_account_receivable_id.id))]
                name = False
...
class account_journal(osv.osv):
    ...
    _columns = {
        ...
        'type': fields.selection([('sale', 'Sale'),('sale_refund','Sale Refund'), ('purchase', 'Purchase'), ('purchase_refund','Purchase Refund'), ('cash', 'Cash'), ('bank', 'Bank and Checks'), ('general', 'General'), ('situation', 'Opening/Closing Situation'), ('stock', _('Stock'))], 'Type', size=32, required=True,
        ...
        'prefix_invoice': fields.char(string='Prefix invoice', size=256, required=False, help="""
        This field can replace the prefix of the sequence if you want to use the date of the invoice instead of the date of the day in the prefix
        Dates:
            ...
            if move.name =='/':
                ...
                if invoice and invoice.internal_number:
                    ...
                    if journal.sequence_id:
                        ...
                        if journal.prefix_invoice:
                            prefix_invoice = datetime.strptime(invoice.date_invoice, '%Y-%m-%d').strftime(journal.prefix_invoice)
                            new_name = '%s%s'%(prefix_invoice, new_name)
                    else:
...
class account_tax(osv.osv):
    ...
    _columns = {
        ...
        'account_id':fields.many2one('account.account', 'Invoice Tax Account', help="Set the account that will be set by default on invoice tax lines for invoices. Leave empty to use the expense account."),
        'refund_account_id':fields.many2one('account.account', 'Refund Tax Account', help="Set the account that will be set by default on invoice tax lines for refunds. Leave empty to use the expense account."),
    ...
    def _compute_amount(self, base_amount, price_unit, quantity=1.0, product=None, partner=None):
        ...
        if (self.type == 'percent' and not self.price_include):
            return base_amount * self.amount / 100
        if self.type == 'percent' and self.price_include:
            return base_amount - (base_amount / (1 + self.amount / 100))
        if self.type == 'division' and not self.price_include:
    ...
    def _compute(self, cr, uid, taxes, price_unit, quantity, product=None, partner=None, precision=None):
        ...
        RETURN:
            [ tax ]
            tax = {'name':'', 'amount':0.0, 'account_id':1, 'refund_account_id':2}
        ...
        for model in self.browse(cr, uid, ids, context=context):
            ...
            for line in model.lines_id:
                ...
                if line.date_maturity == 'partner':
                    ...
                    if model.journal_id.type in ('purchase', 'purchase_refund') and line.partner_id.property_supplier_payment_term_id:
                        payment_term_id = line.partner_id.property_supplier_payment_term_id.id
                    elif line.partner_id.property_payment_term_id:
    ...
    def generate_account(self, cr, uid, chart_template_id, tax_template_ref, acc_template_ref, code_digits, company_id, context=None):
        ...
        for account_template in self.browse(cr, uid, children_acc_template, context=context):
            ...
            vals={
                ...
                'debit_account_type_id': account_template.debit_account_type_id and account_template.debit_account_type_id.id or False,
                'credit_account_type_id': account_template.credit_account_type_id and account_template.credit_account_type_id.id or False,
...
class account_chart_template(osv.osv):
    ...
    _columns={
        ...
        'property_account_receivable_id': fields.many2one('account.account.template', 'Receivable Account'),
        'property_account_payable_id': fields.many2one('account.account.template', 'Payable Account'),
...
class account_tax_template(osv.osv):
    ...
    _columns = {
        ...
        'domain':fields.char('Domain', help="This field is only used if you develop your own module allowing developers to create specific taxes in a custom domain."),
        'account_id':fields.many2one('account.account.template', 'Invoice Tax Account'),
        'refund_account_id':fields.many2one('account.account.template', 'Refund Tax Account'),
    ...
    def _generate_tax(self, cr, uid, tax_templates, tax_code_template_ref, company_id, context=None):
        ...
        for tax in tax_templates:
            ...
            todo_dict[new_tax] = {
                ...
                'refund_account_id': tax.refund_account_id and tax.refund_account_id.id or False,
            }
...
class wizard_multi_charts_accounts(osv.osv_memory):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if 'currency_id' in fields:
            ...
            if company_id:
                ...
                country_id = company_obj.browse(cr, uid, company_id, context=context).partner_id.country_id.id
    ...
    def _prepare_all_journals(self, cr, uid, chart_template_id, acc_template_ref, company_id, context=None):
        ...
        def _get_default_account(journal_type, type='debit'):
            ...
            if journal_type in ('sale', 'sale_refund'):
                default_account = acc_template_ref.get(template.property_account_income_categ_id.id)
            elif journal_type in ('purchase', 'purchase_refund'):
                default_account = acc_template_ref.get(template.property_account_expense_categ_id.id)
    ...
    def _prepare_bank_account(self, cr, uid, line, new_code, acc_template_ref, ref_acc_bank, company_id, context=None):
        ...
        if object_model and object_model == 'account.account.type':
            vals['debit_account_type_id'] = object_id
            vals['credit_account_type_id'] = object_id
