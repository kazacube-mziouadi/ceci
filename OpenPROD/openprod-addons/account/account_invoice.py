...
class account_invoice(models.Model):
    ...
    amount_untaxed_signed = fields.Monetary(string='Untaxed Amount currency', currency_field='company_currency_id',
...
#         return res
    ...
    def onchange_partner_id(self, type, partner_id, date_invoice=False,
            payment_term_id=False, partner_bank_id=False, company_id=False):
        account_id = False
        ...
        if partner_id:
            ...
            if type in ('out_invoice', 'out_refund'):
                ...
                property_payment_term_id = p.property_payment_term_id.id
            else:
                account_id = pay_account.id
                property_payment_term_id = p.property_supplier_payment_term_id.id
        ...
        if payment_term_id != property_payment_term_id:
            if property_payment_term_id:
                to_update = self.onchange_payment_term_date_invoice(property_payment_term_id, date_invoice)
    ...
    def onchange_company_id(self, company_id, part_id, type, invoice_line_ids, currency_id):
        ...
        if company_id and part_id and type:
            ...
            if p.property_account_payable_id and p.property_account_receivable_id and \
                    p.property_account_payable_id.company_id.id != company_id and \
                    p.property_account_receivable_id.company_id.id != company_id:
                prop = self.env['ir.property']
                rec_dom = [('name', '=', 'property_account_receivable_id'), ('company_id', '=', company_id)]
                pay_dom = [('name', '=', 'property_account_payable_id'), ('company_id', '=', company_id)]
...
#             return self.env.ref('account.invoice_form')
    ...
    def action_date_assign(self):
        for inv in self:
            res = inv.onchange_payment_term_date_invoice(inv.payment_term_id.id, inv.date_invoice)
    ...
    def inv_line_characteristic_hashcode(self, invoice_line_ids):
        ...
        return "%s-%s-%s-%s-%s" % (
            ...
            invoice_line_ids.get('tax_code_id', 'False'),
            invoice_line_ids.get('product_id', 'False'),
            invoice_line_ids.get('analytic_account_id', 'False'),
            invoice_line_ids.get('date_maturity', 'False'),
        )
    ...
    def _refund_cleanup_lines(self, lines):
        ...
        for line in lines:
            ...
            if line._name == 'account.invoice.line':
                discount_refund = line.fonction_discount_invoice_refund()
                if discount_refund:
                    values.update(discount_refund)
    ...
    def _prepare_refund(self, invoice, date=None, period_id=None, description=None, journal_id=None):
        ...
        """
        values = {}
        for field in invoice._fields:
            if field not in ['move_id', 'number', 'internal_number']:
    ...
    def pay_and_reconcile(self, pay_amount, pay_account_id, period_id, pay_journal_id,
        ...
        name = name or self.invoice_line_ids[0].name or self.number
...
class account_invoice_line(models.Model):
    ...
    @api.depends('price_unit', 'discount', 'invoice_line_tax_ids', 'quantity',
        'product_id', 'invoice_id.partner_id', 'invoice_id.currency_id', 'correction_amount')
    def _compute_price(self):
        price = self.price_unit * (1 - (self.discount or 0.0) / 100.0)
        taxes = self.invoice_line_tax_ids.compute_all(price, self.currency_id, self.quantity, product=self.product_id, partner=self.invoice_id.partner_id)
        self.price_subtotal = taxes['total_excluded']
        if self.invoice_id:
            self.price_subtotal = (self.invoice_id.currency_id.round(self.price_subtotal)) + self.correction_amount
        ...
        for l in self._context.get('invoice_line_ids', []):
            if isinstance(l, (list, tuple)) and len(l) >= 3 and l[2]:
                ...
                if taxes and len(taxes[0]) >= 3 and taxes[0][2]:
                    ...
                    tax_res = taxes.compute_all(price, False, vals.get('quantity'),
    ...
    def _default_account(self):
        ...
        if self._context.get('type') in ('out_invoice', 'out_refund'):
            return self.env['ir.property'].get('property_account_income_categ_id', 'product.category')
        else:
    ...
    correction_amount = fields.Float(string='Correction amount', default=0.0, required=False, 
                                     help="This amount will be add to the total "
                                          "amount excluding taxes. It allows you to manage the " 
                                          "taxes amount difference when rounded per line or per total")
    ...
    def product_id_change(self, product, uom_id, qty=0, name='', type='out_invoice',
        ...
        if type in ('out_invoice', 'out_refund'):
            account = product.property_account_income or product.categ_id.property_account_income_categ_id
        else:
            account = product.property_account_expense or product.categ_id.property_account_expense_categ_id
        ...
        values['invoice_line_tax_ids'] = fp_taxes.ids
    ...
    def move_line_get(self, invoice_id):
        ...
        for line in inv.invoice_line_ids:
            ...
            taxes = line.invoice_line_tax_ids.compute_all(
                ...
                line.currency_id, line.quantity, line.product_id, inv.partner_id)['taxes']
            for tax in taxes:
    ...
    def onchange_account_id(self, product_id, partner_id, inv_type, fposition_id, account_id):
        ...
        if not product_id:
            ...
            if 'invoice_line_tax_ids' in product_change_result.get('value', {}):
                unique_tax_ids = product_change_result['value']['invoice_line_tax_ids']
        return {'value': {'invoice_line_tax_ids': unique_tax_ids}}
    ...
    def compute(self, invoice):
        ...
        for line in invoice.invoice_line_ids:
            taxes = line.invoice_line_tax_ids.compute_all(
                ...
                line.currency_id, line.quantity, line.product_id, invoice.partner_id)['taxes']
            for tax in taxes:
                val = {
