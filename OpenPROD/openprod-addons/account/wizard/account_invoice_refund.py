        ...
        for form in self.browse(cr, uid, ids, context=context):
            ...
            for inv in inv_obj.browse(cr, uid, context.get('active_ids'), context=context):
                ...
                if mode in ('cancel', 'modify'):
                    ...
                    if mode == 'modify':
                        # Modification de cette partie car on ne sélectionne pas tous les champs créés dans les autres modules de factures. Remplacement de créate par un copie
...
#                         invoice = invoice[0]
#                         del invoice['id']
                        invoice_line_ids = [x.id for x in inv.invoice_line_ids]
                        invoice_lines = inv_line_obj.browse(cr, uid, invoice_line_ids, context=context)
                        invoice_lines = inv_obj._refund_cleanup_lines(cr, uid, invoice_lines, context=context)
                        tax_line_ids = [x.id for x in inv.tax_line_ids]
                        tax_lines = inv_tax_obj.browse(cr, uid, tax_line_ids, context=context)
...
#                             vals_invoice[field] = vals_invoice[field] and vals_invoice[field][0]
                        
#                         inv_id = inv_obj.create(cr, uid, invoice, {})
                        inv_id = inv_obj.copy(cr, uid, inv.id, vals_invoice, {})
                        if inv.payment_term_id.id:
                            data = inv_obj.onchange_payment_term_date_invoice(cr, uid, [inv_id], inv.payment_term_id.id, date)
