...
class jasper_document(orm.Model):
    ...
    _columns = {
        ...
        'param_ids': fields.one2many('jasper.document.parameter',
                                     'document_id', 'Parameters', copy=True),
        ...
        'is_no_display': fields.boolean('Do not display')
    }
    ...
    def create_values(self, cr, uid, id, context=None):
        doc = self.browse(cr, uid, id, context=context)
        if not self.action_values(cr, uid, doc.report_id.id, context=context) and not doc.is_no_display:
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals and 'is_no_display' in vals:
            for doc in self.browse(cr, uid, ids, context=context):
                if doc.is_no_display:
                    self.unlink_values(cr, uid, doc.id, context=context)
                else:
                    self.create_values(cr, uid, doc.id, context=context)
