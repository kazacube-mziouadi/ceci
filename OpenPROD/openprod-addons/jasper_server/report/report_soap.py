...
class Report(object):
    ...
    def __init__(self, name, cr, uid, ids, data, context):
        ...
        self.context = context or self.pool.get('res.users').context_get(cr, uid)
    ...
    def _jasper_execute(self, ex, current_document, js_conf, pdf_list,
        ...
        if not reload_ok:
            ...
            d_par.update({
                ...
                'port_jasper': cny.port_jasper or '',
            })
    ...
    def execute(self):
        ...
        pdf_to_merge_list = [pdf_fo_begin, c, pdf_fo_ended]
        if context.get('pdf_to_merge') and isinstance(context['pdf_to_merge'], list):
            for pdf in context['pdf_to_merge']:
                pdf_to_merge_list.append(pdf)
            
        fo_merge = merge_pdf(pdf_to_merge_list)
