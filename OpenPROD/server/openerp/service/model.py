...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        """
        if not isinstance(___dbname, unicode) and not isinstance(___dbname, str):
            dbname = ___dbname.dbname
            return_dbname = ___dbname 
        else:
            dbname = ___dbname      # NOTE: this forbid to use "___dbname" as arguments in http routes
            return_dbname = ___dbname

        def tr(src, ttype, user_id=False):
            ...
            try:
                ...
                if lang == 'todo':
                    if user_id:
                        ...
                        lang = openerp.registry(dbname).get('res.users') and openerp.registry(dbname)['res.users'].browse(cr, 1, user_id).lang
                    else:
                        return src
        ...
        while True:
            try:
                ...
                return f(return_dbname, *args, **kwargs)
                ...
                for key in registry._sql_error.keys():
                    if key in inst[0]:
                        raise ValidationError(tr(registry._sql_error[key], 'sql_constraint', args and args[0] or False) or inst[0])
...
def exec_workflow_cr(cr, uid, obj, signal, *args, **kwargs):
    res_id = args[0]
    return execute_cr(cr, uid, obj, 'signal_workflow', [res_id], signal)[res_id]


@check
def exec_workflow(db, uid, obj, signal, *args, **kwargs):
    with openerp.registry(db).cursor() as cr:
        return exec_workflow_cr(cr, uid, obj, signal, *args, **kwargs)
