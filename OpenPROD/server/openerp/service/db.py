...
def _initialize_db(id, db_name, demo, lang, user_password):
    try:
        ...
        with closing(db.cursor()) as cr:
            ...
            if not os.path.exists(os.path.join(openerp.tools.config['data_dir'], 'documents')):
                os.mkdir(os.path.join(openerp.tools.config['data_dir'], 'documents'))
            cr.commit()
    except Exception, e:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    old_ds = os.path.join(openerp.tools.config['data_dir'], 'documents', db_original_name)
    new_ds = os.path.join(openerp.tools.config['data_dir'], 'documents', db_name)
    if os.path.exists(from_fs) and not os.path.exists(to_fs):
        shutil.copytree(from_fs, to_fs)
    if os.path.exists(old_ds) and not os.path.exists(new_ds):
        shutil.copytree(old_ds, new_ds)
    return True
...
def exp_drop(db_name):
    ...
    if os.path.exists(fs):
        shutil.rmtree(fs)
    ds = os.path.join(openerp.tools.config['data_dir'], 'documents', db_name)
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with openerp.tools.osutil.tempdir() as dump_dir:
            ...
            documents = os.path.join(openerp.tools.config['data_dir'], 'documents', db_name)
            if os.path.exists(documents):
                shutil.copytree(documents, os.path.join(dump_dir, 'documents'))
    ...
    with openerp.tools.osutil.tempdir() as dump_dir:
        if zipfile.is_zipfile(dump_file):
            ...
            with zipfile.ZipFile(dump_file, 'r') as z:
                ...
                documents = [m for m in z.namelist() if m.startswith('documents/')]
                z.extractall(dump_dir, ['dump.sql'] + filestore + documents)

                if filestore:
                    filestore_path = os.path.join(dump_dir, 'filestore')

                if documents:
                    documents_path = os.path.join(dump_dir, 'documents')
        ...
        with registry.cursor() as cr:
            ...
            if documents_path:
                documents_dest = os.path.join(openerp.tools.config['data_dir'], 'documents', db)
                shutil.move(unicode(documents_path), unicode(documents_dest))
...
def exp_rename(old_name, new_name):
    ...
    old_ds = os.path.join(openerp.tools.config['data_dir'], 'documents', old_name)
    new_ds = os.path.join(openerp.tools.config['data_dir'], 'documents', new_name)
    if os.path.exists(old_ds) and not os.path.exists(new_ds):
        shutil.move(old_ds, new_ds)
    return True
