        ...
        try:
            ...
            if lang:
                ...
                if cr:
                    ...
                    module_name = frame.f_globals['__package__']
                    if module_name.startswith('openerp.addons'):
                        module = module_name[15:].split('.')[0]
                    else:
                        module = None
                    res = registry['ir.translation']._get_source(cr, SUPERUSER_ID, None, ('code','sql_constraint'), lang, source, module=module)
                else:
...
def trans_generate(lang, modules, cr):
    ...
    for module, source, name, id, type, comments in sorted(_to_translate):
        trans = '' if not lang else trans_obj._get_source(cr, uid, name, type, lang, source, module=module)
