...
class test_expression(common.TransactionCase):
    ...
    def test_00_in_not_in_m2m(self):
        ...
        a = partners.create(cr, uid, {'name': 'test_expression_partner_A'})
        b = partners.create(cr, uid, {'name': 'test_expression_partner_B'})
        ab = partners.create(cr, uid, {'name': 'test_expression_partner_AB'})
    ...
    def test_05_not_str_m2m(self):
        ...
        pids = {}
        for p in _partners:
            pids[p] = partners.create(cr, uid, {'name': p, })

        base_domain = [('id', 'in', pids.values())]

        def test(op, value, expected):
            ids = set(partners.search(cr, uid, base_domain))
    ...
    def test_10_expression_parse(self):
        ...
        user_ids = users_obj.search(cr, uid, [('contact_ids.name', 'like', 'test_B')])
        self.assertEqual(set(user_ids), set([b1]), 'searching through inheritance failed')
    ...
    def test_20_auto_join(self):
        ...
        partner_child_ids_col = partner_obj._columns.get('contact_ids')  # one2many on res.partner to res.partner
        ...
        self._reinit_mock()
        partner_ids = partner_obj.search(cr, uid, [('contact_ids.bank_ids.id', 'in', [b_aa, b_ba])])
        # Test result
        self.assertEqual(set(partner_ids), set([p_a, p_b]),
            "_auto_join off: ('contact_ids.bank_ids.id', 'in', [..]): incorrect result")
        # Test produced queries
        self.assertEqual(len(self.query_list), 5,
            "_auto_join off: ('contact_ids.bank_ids.id', 'in', [..]) should produce 5 queries (1 in res_partner_bank, 4 on res_partner)")
        ...
        # Test result
        self.assertEqual(set(partner_ids), set([p_a, p_b]),
            "_auto_join on: ('contact_ids.bank_ids.id', 'not in', [..]): incorrect result")
        # # Test produced queries
        self.assertEqual(len(self.query_list), 1,
            "_auto_join on: ('contact_ids.bank_ids.id', 'in', [..]) should produce 1 query")
        sql_query = self.query_list[0].get_sql()
        self.assertIn('"res_partner"', sql_query[0],
            "_auto_join on: ('contact_ids.bank_ids.id', 'in', [..]) incorrect main table")
        self.assertIn('"res_partner" as "res_partner__child_ids"', sql_query[0],
            "_auto_join on: ('contact_ids.bank_ids.id', 'in', [..]) query incorrect join")
        ...
        self.assertIn('"res_partner__child_ids__bank_ids"."id" in (%s,%s)', sql_query[1],
            "_auto_join on: ('contact_ids.bank_ids.id', 'in', [..]) query incorrect where condition")
        self.assertIn('"res_partner"."id"="res_partner__child_ids"."parent_id"', sql_query[1],
            "_auto_join on: ('contact_ids.bank_ids.id', 'in', [..]) query incorrect join condition")
        ...
        self.assertEqual(set([b_aa, b_ba]), set(sql_query[2][-2:]),
            "_auto_join on: ('contact_ids.bank_ids.id', 'in', [..]) query incorrect parameter")
        ...
        # Do: 2 cascaded one2many with _auto_join, test final leaf is an id
        self._reinit_mock()
        partner_ids = partner_obj.search(cr, uid, ['&', (1, '=', 1), ('contact_ids.bank_ids.id', 'in', [b_aa, b_ba])])
        ...
        self._reinit_mock()
        partner_ids = partner_obj.search(cr, uid, [('contact_ids.state_id.country_id.code', 'like', name_test)])
        # Test result: at least our added data + demo data
        self.assertTrue(set([p_a, p_b]).issubset(set(partner_ids)),
            "_auto_join off: ('contact_ids.state_id.country_id.code', 'like', '..') incorrect result")
        # Test produced queries
        self.assertEqual(len(self.query_list), 5,
            "_auto_join off: ('contact_ids.state_id.country_id.code', 'like', '..') number of queries incorrect")

        partner_child_ids_col._auto_join = True
        partner_state_id_col._auto_join = True
        state_country_id_col._auto_join = True
        self._reinit_mock()
        partner_ids = partner_obj.search(cr, uid, [('contact_ids.country_id.code', 'like', name_test)])
        # Test result: at least our added data + demo data
        self.assertTrue(set([p_a, p_b]).issubset(set(partner_ids)),
            "_auto_join on: ('contact_ids.country_id.code', 'like', '..') incorrect result")
        # Test produced queries
        self.assertEqual(len(self.query_list), 1,
            "_auto_join on: ('contact_ids.country_id.code', 'like', '..') number of queries incorrect")
    ...
    def test_long_table_alias(self):
        ...
        self.patch_order('res.partner', 'company_id,name')
