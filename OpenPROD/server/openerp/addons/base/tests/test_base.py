...
class test_base(common.TransactionCase):
    ...
    def test_50_res_partner_commercial_sync(self):    
        ...
        self.res_partner.write(cr, uid, sunhelm.id, {'contact_ids': [(0, 0, {'name': 'Ulrik Greenthorn',
    ...
    def test_60_read_group(self):
        cr, uid = self.cr, self.uid
        test_users = [
            {'name': 'Alice', 'login': 'alice', 'color': 1, 'date': '2015-03-28'},
            {'name': 'Alice', 'login': 'alice2', 'color': 0, 'date': '2015-01-28'},
            {'name': 'Bob', 'login': 'bob', 'color': 2, 'date': '2015-03-02'},
            {'name': 'Eve', 'login': 'eve', 'color': 3, 'date': '2015-03-20'},
            {'name': 'Nab', 'login': 'nab', 'color': -3, 'date': '2014-09-10'},
            {'name': 'Nab', 'login': 'nab-she', 'color': 6, 'date': '2014-01-02'},
        ]
        ...
        groups_data = self.res_users.read_group(cr, uid, domain, fields=['name', 'color'], groupby=['login'])
        ...
        groups_data = self.res_users.read_group(cr, uid, domain, fields=['color', 'date'], groupby=['date'])
        self.assertEqual(len(groups_data), 4, "Incorrect number of results when grouping on a field")
        ...
        groups_data = self.res_users.read_group(cr, uid, domain, fields=['color', 'date'], groupby=['date'], orderby='date DESC')
        self.assertEqual(len(groups_data), 4, "Incorrect number of results when grouping on a field")
...
class test_partner_recursion(common.TransactionCase):
    ...
    def test_104_res_partner_recursion_indirect_cycle(self):
        ...
        self.assertRaises(ValidationError, self.res_partner.write, cr, uid, [p2],
                          {'contact_ids': [(1, p3, {'parent_id': p3b}), (1, p3b, {'parent_id': p3})]})
