...
class TestAPI(common.TransactionCase):
    ...
    def test_60_cache(self):
        ...
        for p in data:
            pids.append(Partners.create({
                'name': p,
                'contact_ids': [(0, 0, {'name': c}) for c in data[p]],
            }).id)

        partners = Partners.search([('id', 'in', pids)])
        partner1, partner2 = partners[0], partners[1]
        children1, children2 = partner1.contact_ids, partner2.contact_ids
        ...
        self.assertIn(child, partner1.contact_ids)
        self.assertNotIn(child, partner2.contact_ids)
        ...
        self.assertNotIn(child, partner1.contact_ids)
        self.assertIn(child, partner2.contact_ids)
        self.assertEqual(set(partner1.contact_ids + child), set(children1))
        self.assertEqual(set(partner2.contact_ids), set(children2 + child))
        self.env.check_cache()
        ...
        # check recordsets
        self.assertEqual(set(partner1.contact_ids), set(children1) - set([child]))
        self.assertEqual(set(partner2.contact_ids), set(children2))
        self.env.check_cache()
    ...
    def test_80_filter(self):
        ...
        customers = ps.browse([p.id for p in ps if p.is_customer])

        # filter on a single field
        self.assertEqual(ps.filtered(lambda p: p.is_customer), customers)
        self.assertEqual(ps.filtered('is_customer'), customers)

        # filter on a sequence of fields
        self.assertEqual(
            ps.filtered(lambda p: p.parent_id.is_customer),
            ps.filtered('parent_id.is_customer')
        )
