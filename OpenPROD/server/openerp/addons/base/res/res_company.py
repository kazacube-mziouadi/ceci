...
class res_company(osv.osv):
    ...
    _columns = {
        ...
        'port_jasper': fields.integer('Port jasper', required=False),
    ...
    @api.onchange('custom_footer', 'phone', 'email', 'website', 'company_registry')
    def onchange_footer(self):
        if not self.custom_footer:
            ...
            res = ' | '.join(filter(bool, [
                ...
                self.partner_id.fax              and '%s: %s' % (_('Fax'), self.partner_id.fax),
