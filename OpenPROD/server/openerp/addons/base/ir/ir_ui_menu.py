...
class ir_ui_menu(osv.osv):
    ...
    def load_menus(self, cr, uid, debug, context=None):
        ...
        """
        fields = ['name', 'sequence', 'parent_id', 'action', 'web_icon_data', 'description']
        menu_root_ids = self.get_user_roots(cr, uid, context=context)
        menu_roots = self.read(cr, uid, menu_root_ids, fields, context=context) if menu_root_ids else []
        menu_root = {
            'id': False,
    ...
    _columns = {
        ...
        'description_id': fields.many2one('web.page'),
        'description': fields.related('description_id', 'content', type='text', readonly=True),
    }
