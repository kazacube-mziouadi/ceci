...
def apply_rights_modify_view(actions):
    ''' Permet d'être surchargé dans mass_editing pour gérer la sécurité '''
    return actions
...
class BaseModel(object):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _get_default_simple_view(self, cr, user, context=None):
        """ Generates a default single-line simple view using all fields
        of the current model except the m2m and o2m ones.
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        if toolbar:
            ...
            resaction = apply_rights_modify_view(resaction)
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False, lazy=True):
        ...
        aggregated_fields = [
            ...
            # Ajout de la fin de ligne pour permettre la somme sur un text
            if (self._fields[f].type in ('integer', 'float', 'monetary') or getattr(self._fields[f], 'allow_sum', False)) 
            if getattr(self._fields[f].base_field.column, '_classic_write', False)
        ]
        # Debut ajout permettre la somme sur un text
        special_sum = False
        for f in aggregated_fields:
            if getattr(self._fields[f], 'allow_sum', False):
                special_sum = True
                break
        # Fin ajout permettre la somme sur un text
        ...
        if special_sum:
            ...
            select_terms = ["%s(CASE WHEN %s ~ '^[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?$' THEN %s::float ELSE 0 END) AS %s" % field_formatter(f) for f in aggregated_fields]
        else:
        # Fin ajout permettre la somme sur un text
            field_formatter = lambda f: (
                self._fields[f].group_operator or 'sum',
    ...
    def _write(self, cr, user, ids, vals, context=None):
        ...
        if updates:
            ...
            for f in direct:
                ...
                elif has_trans and column.translate and not isinstance(column, openerp.osv.fields.html) and not isinstance(column,openerp.osv.fields.char):
            ...
            # and for translatable fields we keep them for copy
                ...
                for record in trans_obj.read(cr, uid, trans_ids, context=context):
                    ...
                    if user_lang and user_lang == record['lang']:
                        ...
                        #===============================================================================================
                        # Remettre old_record à la ligne suivante si des problèmes de copy_translation sont constatés
                        #===============================================================================================
