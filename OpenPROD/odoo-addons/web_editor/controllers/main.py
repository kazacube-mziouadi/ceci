...
class Web_Editor(http.Controller):
    ...
    @http.route('/web_editor/get_translation', type='http', auth='user', methods=['GET'])
    def get_translation(self, lang, field, model, res_id):
        request.context['lang'] = lang
        res_id = int(res_id)
        return request.registry[model].read(request.cr, request.uid, res_id, context=request.context)[field]

    @http.route('/web_editor/set_translation', type='json', auth="user", methods=['POST'], csrf=False)
    def set_translation(self, langues, field, model, res_id, contents):
        i = 0
        for lang in langues:
            mail_template = http.request.env['ir.translation'].with_context(lang=lang).search([ ('res_id', '=', int(res_id)), ('name', '=', 'mail.template,'+field), ('lang' , '=' , lang) ])
            if mail_template :
                mail_template.with_context(lang=lang).write({ 'value' : contents[i] })
            else :
                src = http.request.env['mail.template'].sudo().browse(int(res_id))
                values = { 'name': 'mail.template,'+field,
                        'type': 'model',  
                        'state': 'to_translate', 
                        'lang': lang, 
                        'res_id': int(res_id),
                        'src': src.body_html,
                        'value': contents[i],}
                http.request.env['ir.translation'].sudo().create(values)   
            i = i +1
        return "Success"
