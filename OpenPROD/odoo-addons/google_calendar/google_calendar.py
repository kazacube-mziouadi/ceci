...
class google_calendar(osv.AbstractModel):
    ...
    def generate_data(self, cr, uid, event, isCreating=False, context=None):
        if not context:
            context = {}
        start_date = fields.datetime.context_timestamp(cr, uid, datetime.strptime(event.start_datetime, tools.DEFAULT_SERVER_DATETIME_FORMAT), context=context).isoformat('T')
        final_date = fields.datetime.context_timestamp(cr, uid, datetime.strptime(event.stop_datetime, tools.DEFAULT_SERVER_DATETIME_FORMAT), context=context).isoformat('T')
        type = 'dateTime'
        vstype = 'date'
        attendee_list = []
        for attendee in event.attendee_ids:
            ...
            attendee_list.append({
                'email': email,
                'displayName': attendee.user_id.name,
    ...
    def create_from_google(self, cr, uid, event, user_id, context=None):
        ...
        attendee_record_id = self.pool['calendar.attendee'].search(cr, uid, [('user_id', '=', user_id), ('event_id', '=', res)], context=context)
    ...
    def update_from_google(self, cr, uid, event, single_event_dict, type, context):
        ...
        myPartnerID = user_obj.browse(cr, uid, uid, context).id
        attendee_record = []
        alarm_record = set()
        result = {}
        ...
        if type == "write":
            ...
            action_google_id = self.pool['action.type'].search(cr, uid, [('name', '=', 'Google')])
            if action_google_id:
                result['type_id'] = action_google_id
    ...
    def remove_references(self, cr, uid, context=None):
        ...
        all_my_attendees = self.pool['calendar.attendee'].search(cr, uid, [('user_id', '=', current_user.id)], context=context)
    ...
    def create_new_events(self, cr, uid, context=None):
        ...
        user_obj = self.pool['res.users']
        myPartnerID = user_obj.browse(cr, uid, uid, context=context).id

        context_norecurrent = context.copy()
        context_norecurrent['virtual_id'] = False
        my_att_ids = att_obj.search(cr, uid, [('user_id', '=', myPartnerID),
                                    ('google_internal_event_id', '=', False),
                                    '|',
                                    ('event_id.stop_datetime', '>', self.get_minTime(cr, uid, context=context).strftime(DEFAULT_SERVER_DATETIME_FORMAT)),
    ...
    def bind_recurring_events_to_google(self, cr, uid, context=None):
        ...
        my_att_ids = att_obj.search(cr, uid, [('user_id', '=', myPartnerID), ('google_internal_event_id', '=', False)], context=context_norecurrent)
        ...
        for att in att_obj.browse(cr, uid, my_att_ids, context=context):
            if att.event_id.recurrent_id and att.event_id.recurrent_id > 0:
                ...
                source_attendee_record_id = att_obj.search(cr, uid, [('user_id', '=', myPartnerID), ('event_id', '=', source_event_record.id)], context=context)
    ...
    def update_events(self, cr, uid, lastSync=False, context=None):
        ...
        if lastSync:
            ...
            domain = [
                ...
                ('google_internal_event_id', '!=', False),
                '|',
                ('event_id.stop_datetime', '>', self.get_minTime(cr, uid, context).strftime(DEFAULT_SERVER_DATETIME_FORMAT)),
...
class calendar_attendee(osv.Model):
    ...
    _sql_constraints = [('google_id_uniq', 'unique(google_internal_event_id,user_id,event_id)', 'Google ID should be unique!')]
