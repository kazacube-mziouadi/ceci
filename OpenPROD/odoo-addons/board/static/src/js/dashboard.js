...
FavoriteMenu.include({
    prepare_dropdown_menu: function (filters) {
        ...
        if (am && am.get_inner_widget() instanceof ViewManager) {
	        ...
	        this.$add_dashboard_select = this.$add_to_dashboard.eq(1).find('select');
	        this.dashboard_list = new Model('ir.actions.act_window')
	                                           .query()
	                                           .filter([['res_model', '=', 'board.board']])
	                                           .all().then($.proxy(this.add_board_list, this));
    ...
    add_dashboard: function () {
        ...
        return self.rpc('/board/add_to_dashboard', {
            ...
            name: name,
            dashboard_action_id: parseInt(self.$add_dashboard_select.val()),
        }).then(function (r) {
            if (r) {
    ...
    add_board_list: function(boards) {
        var self = this;
        _.each(boards, function(board) {
                var option = self.$add_dashboard_select.append('<option value="'+board.id+'">'+board.name+'</option>');
                option.data(board);
            });
    },
});
