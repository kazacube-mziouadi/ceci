...
class Board(Controller):
    ...
    def add_to_dashboard(self, action_id, context_to_save, domain, view_mode, name='', dashboard_action_id=None):
        # Retrieve the 'My Dashboard' action from its xmlid
        if dashboard_action_id:
                action = request.env['ir.actions.act_window'].browse(dashboard_action_id)
        else:
