...
class DiagramView(openerp.http.Controller):
    ...
    def get_diagram_info(self, req, id, model, node, connector,
        ...
        visible_node_fields = kw.get('visible_nodes',[])
        invisible_node_fields = kw.get('invisible_nodes',[])
        node_fields_string = kw.get('node_fields',[])
        connector_fields = kw.get('connectors',[])
        connector_fields_string = kw.get('connectors_fields',[])
        ...
        for tr in data_connectors:
            ...
            for i, fld in enumerate(connector_fields):
                t['options'][connector_fields[i]] = tr[fld]
        ...
        for act in data_acts:
            ...
            if act.get('position_x', False) or act.get('position_y', False):
                n.update(
                          x=act['position_x'],
                          y=act['position_y'],
                          )
