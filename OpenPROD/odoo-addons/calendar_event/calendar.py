...
class calendar_attendee(osv.Model):
    ...
    def _compute_data(self, cr, uid, ids, name, arg, context=None):
        ...
        for attdata in self.browse(cr, uid, ids, context=context):
            ...
            if name == 'cn':
                if attdata.user_id:
                    result[id][name] = attdata.user_id.name or False
                else:
    ...
    _columns = {
        ...
        'user_id': fields.many2one('res.users', 'Contact', readonly="True"),
    ...
    def get_ics_file(self, cr, uid, event_obj, context=None):
        ...
        if not event_obj.start_datetime or not event_obj.stop_datetime:
            raise UserError(_("First you have to specify the date of the invitation."))
        event.add('created').value = ics_datetime(time.strftime(DEFAULT_SERVER_DATETIME_FORMAT))
        event.add('dtstart').value = ics_datetime(event_obj.start_datetime)
        event.add('dtend').value = ics_datetime(event_obj.stop_datetime)
    ...
    def _send_mail_to_attendees(self, cr, uid, ids, email_from=tools.config.get('email_from', False),
                                template_xmlid='calendar_template_meeting_invitation2', force=False, context=None):
        """
        ...
        dummy, template_id = data_pool.get_object_reference(cr, uid, 'calendar_event', template_xmlid)
        dummy, act_id = data_pool.get_object_reference(cr, uid, 'calendar_event', "view_calendar_event_calendar")
        ...
        for attendee in self.browse(cr, uid, ids, context=context):
            if attendee.email and email_from and (attendee.email != email_from or force):
                ...
                mail_id = template_pool.send_mail(cr, uid, template_id, attendee.id, email_to=attendee.email, context=local_context)
...
class res_users(osv.Model):
    ...
    def get_attendee_detail(self, cr, uid, ids, meeting_id, context=None):
        ...
        for user in self.browse(cr, uid, ids, context=context):
            data = self.name_get(cr, uid, [user.id], context)[0]
            data = [data[0], data[1], False, user.color]
            if meeting:
    ...
    def _set_calendar_last_notif_ack(self, cr, uid, context=None):
        ...
        self.write(cr, uid, user.id, {'calendar_last_notif_ack': datetime.now()}, context=context)
        return
...
class calendar_alarm_manager(osv.AbstractModel):
    ...
    def get_next_potential_limit_alarm(self, cr, uid, seconds, notif=True, mail=True, user_id=None, context=None):
        res = {}
        base_request = """
                    SELECT
                        cal.id,
        ...
        filter_user = """
                RIGHT JOIN calendar_event_res_user_rel AS part_rel ON part_rel.calendar_event_id = cal.id
                    AND part_rel.res_users_id = %s
        """
    ...
    def get_next_mail(self, cr, uid, context=None):
        ...
        for curEvent in self.pool.get('calendar.event').browse(cr, uid, all_events.keys(), context=context):
            ...
            if curEvent.recurrency:
                ...
                in_date_format = datetime.strptime(curEvent.start_datetime, DEFAULT_SERVER_DATETIME_FORMAT)
                last_found = self.do_check_alarm_for_one_date(cr, uid, in_date_format, curEvent, max_delta, 0, after=last_notif_mail, notif=False, missing=True, context=context)
                for alert in last_found:
                    self.do_mail_reminder(cr, uid, alert, context=context)
    ...
    def get_next_notif(self, cr, uid, context=None):
        ...
        user = self.pool['res.users'].read(cr, SUPERUSER_ID, uid, ['calendar_last_notif_ack'], context=context)
        all_notif = []

        if not user:
            return []

        all_events = self.get_next_potential_limit_alarm(cr, uid, ajax_check_every_seconds, user_id=uid, mail=False, context=context)
        ...
        for event in all_events:  # .values()
            ...
            if curEvent.recurrency:
                ...
                for one_date in self.pool.get("calendar.event").get_recurrent_date_by_event(cr, uid, curEvent, context=context):
                    in_date_format = one_date.replace(tzinfo=None)
                    LastFound = self.do_check_alarm_for_one_date(cr, uid, in_date_format, curEvent, max_delta, ajax_check_every_seconds, after=user['calendar_last_notif_ack'], mail=False, context=context)
                    if LastFound:
    ...
    def do_mail_reminder(self, cr, uid, alert, context=None):
        ...
        if alarm.type == 'email':
            res = self.pool['calendar.attendee']._send_mail_to_attendees(
                ...
                email_from=event.user_id.email,
                template_xmlid='calendar_template_meeting_reminder2',
                force=True,
                context=context
            )
...
class calendar_event(osv.Model):
    ...
    def get_recurrent_date_by_event(self, cr, uid, event, context=None):
        ...
        startdate = pytz.UTC.localize(datetime.strptime(event.start_datetime, DEFAULT_SERVER_DATETIME_FORMAT))  # Add "+hh:mm" timezone
        if not startdate:
    ...
    def _get_recurrency_end_date(self, cr, uid, id, context=None):
        data = self.read(cr, uid, id, ['final_date', 'recurrency', 'rrule_type', 'count', 'end_type', 'stop_datetime'], context=context)
        ...
        if end_type == 'count' and all(data.get(key) for key in ['count', 'rrule_type', 'stop_datetime']):
            count = data['count'] + 1
            delay, mult = {
                'daily': ('days', 1),
                'weekly': ('days', 7),
            ...
            deadline = datetime.strptime(data['stop_datetime'], tools.DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def _find_my_attendee(self, cr, uid, meeting_ids, context=None):
        ...
        for meeting_id in meeting_ids:
            for attendee in self.browse(cr, uid, meeting_id, context).attendee_ids:
                if user.id == attendee.user_id.id:
                    return attendee
        return False
    ...
    def get_display_time_tz(self, cr, uid, ids, tz=False, context=None):
        ...
        return self._get_display_time(cr, uid, ev.start_datetime, ev.stop_datetime, ev.duration, context=context)

    def _get_display_time(self, cr, uid, start, stop, zduration, context=None):
        """
        ...
        for meeting in self.browse(cr, uid, ids, context=context):
            ...
            for field in fields:
                if field == 'is_attendee':
                    ...
                    meeting_data[field] = self._get_display_time(cr, uid, meeting.start_datetime, meeting.stop_datetime, meeting.duration, context=context)
                elif field == "display_start":
                    meeting_data[field] = meeting.start_datetime
        return res
    ...
    def _set_date(self, cr, uid, values, id=False, context=None):
        ...
        if values.get('start_datetime') or values.get('stop_datetime'):
            diff = False
    ...
    _columns = {
        ...
        'color_user_id': fields.related('user_id', 'id', type="integer", string="Color index of creator", store=False),  # Color of creator
        ...
        'user_ids': fields.many2many('res.users', 'calendar_event_res_user_rel', 'calendar_event_id', 'res_users_id', string='Attendees', states={'done': [('readonly', True)]}),
        'alarm_ids': fields.many2many('calendar.alarm', 'calendar_alarm_calendar_event_rel', string='Reminders', ondelete="restrict", copy=False),
    }

    def _get_default_users(self, cr, uid, ctx=None):
        ret = [self.pool['res.users'].browse(cr, uid, uid, context=ctx).id]
        active_id = ctx.get('active_id')
        if ctx.get('active_model') == 'res.users' and active_id:
    ...
    def _check_closing_date(self, cr, uid, ids, context=None):
        for event in self.browse(cr, uid, ids, context=context):
            if event.stop_datetime < event.start_datetime:
                return False
        return True
    ...
    def onchange_duration(self, cr, uid, ids, start_type='start', start=False, duration=False, context=None):
        value = {}
        if not start:
            return value
        ...
        elif start_type == 'stop':
            stop = datetime.strptime(start, DEFAULT_SERVER_DATETIME_FORMAT)
            value['start_datetime'] = (stop - timedelta(hours=duration)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def new_invitation_token(self, cr, uid, record, user_id):
    ...
    def create_attendees(self, cr, uid, ids, context=None):
        ...
        for event in self.browse(cr, uid, ids, context):
            ...
            for att in event.attendee_ids:
                attendees[att.user_id.id] = True
            new_attendees = []
            new_att_user_ids = []
            for user in event.user_ids:
                if user.id in attendees:
                    continue
                access_token = self.new_invitation_token(cr, uid, event, user.id)
                values = {
                    'user_id': user.id,
                    'event_id': event.id,
                ...
                new_att_user_ids.append(user.id)

                if not current_user.email or current_user.email != user.email:
            ...
            # We remove old attendees who are not in user_ids now.
            all_user_ids = [part.id for part in event.user_ids]
            all_part_attendee_ids = [att.user_id.id for att in event.attendee_ids]
            all_attendee_ids = [att.id for att in event.attendee_ids]
            user_ids_to_remove = map(lambda x: x, set(all_part_attendee_ids + new_att_user_ids) - set(all_user_ids))

            attendee_ids_to_remove = []

            if user_ids_to_remove:
                attendee_ids_to_remove = self.pool["calendar.attendee"].search(cr, uid, [('user_id.id', 'in', user_ids_to_remove), ('event_id.id', '=', event.id)], context=context)
    ...
    def get_recurrent_ids(self, cr, uid, event_id, domain, order=None, context=None):
        ...
        if order_fields:
            ...
            sort_params = uniq([comp if comp != 'start_datetime' else 'sort_start' for comp in sort_params])
            sort_params = uniq([comp if comp != '-start_datetime' else '-sort_start' for comp in sort_params])
    ...
    def _track_subtype(self, cr, uid, ids, init_values, context=None):
        ...
        if 'start' in init_values and record.start_datetime:
    ...
    def onchange_user_ids(self, cr, uid, ids, value, context=None):
        ...
        res.update(self.check_users_email(cr, uid, value[0][2], context=context))
        return res

    def check_users_email(self, cr, uid, user_ids, context=None):
        """ Verify that selected user_ids have an email_address defined.
            Otherwise throw a warning. """
        user_wo_email_lst = []
        for user in self.pool['res.users'].browse(cr, uid, user_ids, context=context):
            if not user.email:
    ...
    def _needaction_domain_get(self, cr, uid, context=None):
        return [
            ('stop_datetime', '<=', time.strftime(DEFAULT_SERVER_DATE_FORMAT + ' 23:59:59')),
            ('start_datetime', '>=', time.strftime(DEFAULT_SERVER_DATE_FORMAT + ' 00:00:00')),
            ('user_id', '=', uid),
        ]
...
#                     self.message_post(cr, uid, event.id, body=_("An invitation email has been sent to attendee(s)"), subtype="calendar.subtype_invitation", context=context)
    ...
    def search(self, cr, uid, args, offset=0, limit=0, order=None, context=None, count=False):
        ...
        if context.get('mymeetings', False):
            user_id = self.pool['res.users'].browse(cr, uid, uid, context).id
            args += [('user_ids', 'in', [user_id])]

        new_args = []
        for arg in args:
            new_arg = arg

            if arg[0] == 'stop_datetime' and arg[1] == ">=":
    ...
    def _detach_one_event(self, cr, uid, id, values=dict(), context=None):
        ...
        data = self.read(cr, uid, id, ['start_datetime', 'stop_datetime', 'rrule', 'duration'])
        if data.get('rrule'):
            data.update(
                values,
                recurrent_id=real_event_id,
                recurrent_id_date=data.get('start_datetime'),
                rrule_type=False,
                rrule='',
                recurrency=False,
                final_date=datetime.strptime(data.get('start_datetime'), DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(hours=values.get('duration', False) or data.get('duration'))
            )
    ...
    def write(self, cr, uid, ids, values, context=None):
        ...
        for event_id in ids:
            ...
            if '-' not in str(event_id):
                ...
                blacklisted = any(key in values for key in ('start_datetime', 'stop_datetime', 'active'))
                if not values.get('recurrency', True) or not blacklisted:
                    real_ids = [real_event_id]
                else:
                    data = self.read(cr, uid, event_id, ['start_datetime', 'stop_datetime', 'rrule', 'duration'])
                    if data.get('rrule'):
            ...
            if values.get('recurrency') and values.get('end_type', 'count') in ('count', unicode('count')) and \
                    (values.get('rrule_type') or values.get('count') or values.get('start_datetime') or values.get('stop_datetime')):
                for id in real_ids:
            ...
            if values.get('start_datetime') and values.get('active', True):
                for the_id in real_ids + new_ids:
                    ...
                    if mail_to_ids:
                        current_user = self.pool['res.users'].browse(cr, uid, uid, context=context)
                        self.pool['calendar.attendee']._send_mail_to_attendees(cr, uid, mail_to_ids, template_xmlid='calendar_template_meeting_changedate2', email_from=current_user.email, context=context)
    ...
    def read(self, cr, uid, ids, fields=None, context=None, load='_classic_read'):
        ...
        EXTRAFIELDS = ('class', 'user_id', 'duration', 'start_datetime', 'rrule')
        ...
        for calendar_id, real_id in select:
            ...
            if not isinstance(ls, (basestring, int, long)) and len(ls) >= 2:
                ...
                if 'display_time' in fields:
                    res['display_time'] = self._get_display_time(cr, uid, ls[1], ls[2], res['duration'], context=context)
        ...
        for r in result:
            ...
            if r['class'] == 'private':
                for f in r.keys():
                    recurrent_fields = self._get_recurrent_fields(cr, uid, context=context)
                    public_fields = list(set(recurrent_fields + ['id', 'start_datetime', 'stop_datetime', 'duration', 'user_id', 'state', 'interval', 'count', 'recurrent_id_date', 'rrule']))
