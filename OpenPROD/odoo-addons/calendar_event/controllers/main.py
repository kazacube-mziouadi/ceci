...
class meeting_invitation(http.Controller):
    ...
    def view(self, db, token, action, id, view='calendar'):
        ...
        with registry.cursor() as cr:
            ...
            if attendee and attendee[0] and attendee[0].get('user_id'):
                user_id = int(attendee[0].get('user_id')[0])
                tz = user_pool.read(cr, openerp.SUPERUSER_ID, user_id, ['tz'])['tz']
            else:
                tz = False
    ...
    def notify_ack(self, type=''):
        ...
        context = request.session.context
        with registry.cursor() as cr:
            res = registry.get("res.users")._set_calendar_last_notif_ack(cr, uid, context=context)
            return res
