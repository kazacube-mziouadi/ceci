...
class Home(http.Controller):
    ...
    def web_client(self, s_action=None, **kw):
        ...
        favorite_ids = request.registry['ir.filters'].search(request.cr, request.uid, [
            ('user_id', '=', request.uid),
        ])
        favorites = request.registry['ir.filters'].browse(request.cr, request.uid, favorite_ids)
        return request.render('web.webclient_bootstrap', qcontext={
            'menu_data': menu_data,
...
class Session(http.Controller):
    ...
    def session_info(self):
        ...
        return {
            ...
            "registered_contract": request.env['maintenance.contract.openprod'].search([('state', '=', 'active'),
                                                                               ('start_date', '<=', openerp.fields.Date.today()),
                                                                               ('end_date', '>=', openerp.fields.Date.today())], limit=1).contract_number
        }
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                with registry.cursor() as cr:
                    cr.execute("""SELECT c.logo, c.write_date
                                    FROM res_users u
...
class Action(http.Controller):
    ...
    def load(self, action_id, do_not_eval=False, additional_context=None):
        ...
        if value.get('res_model'):
            sec_rule, model_id = request.session.model('export.security.rule').get_security_rule(value['res_model'], http.request.env.user)
            if sec_rule:
                value['security_rule'] = sec_rule
...
class ExportFormat(object):
    ...
    def base(self, data, token):
        ...
        model, fields, ids, domain, import_compat, header_type = \
            operator.itemgetter('model', 'fields', 'ids', 'domain',
                                'import_compat', 'header_type')(
                params)
        ...
        export_security_rule_obj = http.request.env['export.security.rule']
        export_security_rule = export_security_rule_obj.search([
            ('model_id.model', '=', model), ('criticality', '!=', 'none'), ('trace_export', '=', True)]
        )
        if len(export_security_rule):
            http.request.env['export.security.log'].create({
                'model': model,
                'user_id': http.request.session._uid,
                'criticality': export_security_rule.criticality,
                'name': 'Export',
                'filter': json.dumps(domain),
                'group_by': [],
                'lines_count': len(import_data),
                 })
...
class Reports(http.Controller):
    ...
    @serialize_exception
    def index(self, action, token, domain='', groupby=''):
        ...
        target_model = action.get('model', action['context'].get('active_model'))
        export_security_rule_obj = http.request.env['export.security.rule']
        export_security_rule = export_security_rule_obj.search([
            ('model_id.model', '=', target_model), ('criticality', '!=', 'none'), ('trace_report', '=', True)]
        )
        if len(export_security_rule):
            http.request.env['export.security.log'].create({
                'model': target_model,
                'user_id': http.request.session._uid,
                'criticality': export_security_rule.criticality,
                'name': 'Report %s' % action.get('name'),
                'filter': domain,
                'group_by': groupby,
                'lines_count': len(report_ids),
                 })
