...
var ActionManager = Widget.extend({
    ...
    dialog_stop: function (reason) {
        if (this.dialog_stack.length > 0) {
            var dialog = this.dialog_stack[this.dialog_stack.length - 1];
        ...
        return this.webclient.clear_uncommitted_changes().then(function() {
            ...
            return $.when(action.restore(index)).done(function() {
                if (action !== old_action) {
                    ...
                    self.inner_action.set_is_in_DOM(true);
                    $('header.o_statusbar_buttons').width( $('header.o_statusbar_buttons').parent().width() + 20);
                }
            });
        }).fail(function() {
            return $.Deferred().reject();
    ...
    do_action: function(action, options) {
        ...
        // Special case for simple view
        if( action.view_mode === 'simple'){
            _.extend(action.flags, {
                headless: true,
                views_switcher: false,
                display_title: false,
            ...
            var popup = action.target === 'new' || action.target === 'stack';
    ...
    ir_actions_common: function(executor, options) {
        if (executor.action.target === 'stack') {
            ...
            dialog.on_close = function(){
                ...
                self.dialog_widget_stack.pop();
            };
            dialog.on("closed", null, dialog.on_close);
            var dialog_widget = executor.widget();

            if (dialog_widget instanceof ViewManager) {
                _.extend(dialog_widget.flags, {
                    $buttons: dialog.$footer,
                    footer_to_buttons: true,
                });
                if (dialog_widget.action.view_mode === 'form') {
                    dialog_widget.flags.headless = true;
                }
            }
            if (dialog_widget.need_control_panel) {
                // Set a fake bus to Dialogs needing a ControlPanel as they should not
                // communicate with the main ControlPanel
                dialog_widget.set_cp_bus(new core.Bus());
            }

            dialog_widget.setParent(dialog);
            dialog.open()
            
            this.dialog_stack.push(dialog);
            this.dialog_widget_stack.push(dialog_widget);
            
            return dialog_widget.appendTo(dialog.$el);
        }
        if (executor.action.target === 'new') {
            ...
            this.dialog = new Dialog(this, {
                ...
                dialogClass: executor.klass,
                buttons: [],
                size: executor.action.context.dialog_size || 'large',
            });
        ...
        return def.then(function() {
            ...
            return self.push_action(executor.widget(), executor.action, options).then(function(){
    ...
    ir_actions_act_window: function (action, options) {
        var self = this;
        // /!\ Execute post Dialog close method
        if (action.context.on_close_method) {
            options.on_close = _.wrap(options.on_close, function(f) {
                var Model = require('web.Model');
                var model = new Model(self.inner_action.action_descr.res_model);
                model.call(
                    action.context.on_close_method,
                    [self.inner_widget.dataset.ids[self.inner_widget.dataset.index]]
                    );
                f();
            })
        }
    ...
    ir_actions_act_window_view_reload: function(action, options) {
    ...
    ir_actions_report_xml: function(action, options) {
        ...
        var domain = _.reduce(
            _.map(
                self.get_inner_widget().searchview.build_search_data().domains,
                function(domain){
            ...
            []);
            domain.concat(self.get_inner_widget().dataset.domain);
            self.session.get_file({
                url: '/web/report',
                data: {action: JSON.stringify(action), domain:domain, groupby:self.get_inner_widget().searchview.build_search_data().groupbys},
