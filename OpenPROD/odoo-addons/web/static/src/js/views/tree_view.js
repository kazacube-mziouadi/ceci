...
var TreeView = View.extend(/** @lends instance.web.TreeView# */{
    ...
    hook_row_click: function () {
        ...
        this.$el.delegate('.treeview-tr', 'click', function () {
            ...
            _(children_ids).each(function(childid) {
                var sib = $this.parent().next();
                var $el = $();
                while (sib.data('level') == $this.parent().data('level') + 1) {
                    if (sib.data("row-parent-id") == record_id && sib.attr('id') == 'treerow_' + childid) {
                        $el = sib;
                    }
                    sib = sib.next();
                }
    ...
    showcontent: function (curnode,record_id, show) {
        ...
        var current_level = curnode.parent('tr').data('level');
        var $row = curnode.parent('tr').next();
        while ($row.data('level') > current_level) {
            if (!show || show && $row.data('level') == current_level + 1) {
                if ($row.hasClass('oe_open')) {
                    $row.toggleClass('oe_open',show);
                }
                $row.toggle(show);
            }
