...
var FormView = View.extend(common.FieldManagerMixin, {
    ...
    init: function(parent, dataset, view_id, options) {
        var self = this;
        this._super(parent);
        this.previous_wkf_id = -1;   // utilisé lors de la mise à jour du wkf_id pour renvoyer un field view get
        this.previous_model = "";    // utilisé lors de la mise à jour du wkf_id pour renvoyer un field view get
    ...
    render_buttons: function($node) {
    	...
    	this.$buttons.on('click', '.oe_form_button_openprod_delete',
    			this.guard_active(this.on_button_delete));
    	this.$buttons.on('click', '.oe_form_button_openprod_duplicate',
...
//            ]));
    ...
    load_record: function(record) {
        // si le wkf_id du modèle change on recharge la fenetre pour avoire le bon field_view_get
        if (record.wkf_id){
            if (record.wkf_id[0] !== this.previous_wkf_id && this.model === this.previous_model){
                window.location.reload();
            }
            // mise-à-jour du modèle et du field view get
            this.previous_model = this.model;
            this.previous_wkf_id = record.wkf_id[0]; 
        }
        ...
        _(this.fields).each(function (field, f) {
            ...
            var result = field.set_value(self.datarecord[f] || self.datarecord[f] === 0 ? self.datarecord[f] : false);
        ...
        return $.when.apply(null, set_values).then(function() {
            ...
            // cache le lien changer de mot de passe si l'utilisateur n'est pas authorisé à changer son mot de passe
            if (self.model=='res.users' && self.$el.find('.button_change_password')[0]){
                console.log(self.get_fields_values().can_change_password);
                if (self.get_fields_values().can_change_password === false ){
                    self.$el.find('.button_change_password')[0].style.display = 'none';
                }
                else{
                    console.log(self.$el.find('.button_change_password')[0]);
                    self.$el.find('.button_change_password')[0].style.display = '';
                }
            }
            $('.oe_application').show();
            self.autofocus();
...
var FormRenderingEngine = FormRenderingEngineInterface.extend({
    ...
    render_to: function($target) {
        ...
        // On insère de l'espace car le header est en postition fixed
        if (this.$form[0].children[0].children[0].nodeName == 'HEADER') {
            var spaceDiv = document.createElement("div");
            spaceDiv.style.height = '17px';
            this.$form[0].children[0].children[1].parentNode.insertBefore( spaceDiv , this.$form[0].children[0].children[1]  ) ;
        }
    ...
    process_form: function($form) {
        ...
        if ($form.attr('readonly')) {
            $form.children().each(function (i, el) {
                var $el = $(el);
                var mod = JSON.parse($el.attr('modifiers'));
                mod.readonly = 1;
                $el.attr('modifiers', JSON.stringify(mod));
            })
        }

        if ($form.attr('modifiers')) {
            var attrs = $form.attr('modifiers');
            var readonly = JSON.parse(attrs);
            if (readonly) {
                $form.children().each(function (i, child) {
                    var child_attrs = ($(child).attr('modifiers') && JSON.parse($(child).attr('modifiers'))) || false;
                    if (!child_attrs) {
                        child_attrs = {}
                        child_attrs.readonly = readonly;
                    } else {
                        if (child_attrs.readonly) {
                            child_attrs.readonly = child_attrs.readonly;
                        } else {
                            child_attrs.readonly = readonly.readonly;
                        }
                    }
    ...
    */
    ...
    process_group: function($group) {
        ...
        if ($group.attr('modifiers')) {
            var attrs = $group.attr('modifiers');
            var readonly = JSON.parse(attrs);
            if (readonly) {
                $group.children().each(function (i, child) {
                    var child_attrs = ($(child).attr('modifiers') && JSON.parse($(child).attr('modifiers'))) || false;
                    if (!child_attrs) {
                        child_attrs = {};
                        child_attrs.readonly = readonly.readonly;
    ...
    process_notebook: function($notebook) {
        ...
        var pages = [];
        var notebook_readonly = $notebook.attr('modifiers') && JSON.parse($notebook.attr('modifiers')).readonly;
        ...
        $notebook.find('> page').each(function() {
            ...
            var page_modifiers = page_attrs.modifiers && JSON.parse(page_attrs.modifiers);

            if (notebook_readonly) {
                if (!page_attrs.modifiers) {
                    page_modifiers = {};
                    page_modifiers.readonly = notebook_readonly;
                } else {
                    if (page_modifiers.readonly) {
                        page_modifiers.readonly = page_modifiers.readonly;
                    } else {
                        page_modifiers.readonly = notebook_readonly;
                    }
                }
                $page.attr('modifiers', JSON.stringify(page_modifiers));
                page_attrs = $page.getAttributes();
            }
            
            if ('readonly' in page_attrs) {
                $page.children().each(function (i, el) {
                    var $el = $(el);
                    var mod = ($el.attr('modifiers') && JSON.parse($el.attr('modifiers'))) || {};
                    mod.readonly = 1;
                    $el.attr('modifiers', JSON.stringify(mod));
                })
            }

            if ('modifiers' in page_attrs) {
                var readonly = page_attrs.modifiers;
                if (readonly) {
                    readonly = JSON.parse(readonly);
                    $page.children().each(function (i, child) {
                        var child_attrs = ($(child).attr('modifiers') && JSON.parse($(child).attr('modifiers'))) || false;
                        if (!child_attrs) {
                            child_attrs = {};
                            child_attrs.readonly = readonly.readonly;
        ...
        _.each(pages, function(page) {
            ...
            if (!pageid_to_display && page.autofocus == 'autofocus') {
