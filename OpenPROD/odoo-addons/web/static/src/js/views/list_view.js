...
var Inputs = require('web.search_inputs');
...
var ListView = View.extend( /** @lends instance.web.ListView# */ {
    ...
    events: {
        ...
        'change .oe_list_header_search input': 'change_list_filters',
        'change .oe_list_header_search select[type]': 'change_list_filters',
        'mouseleave .ms-drop' : 'mouse_leave_ms_drop'  
    },
    /**
     * Fonction surchargé dans web_multi_select pour lancer la recherche quand la souris de l'utilisateur quitte 
     * la drop-down
     */
    ...
    view_loading: function(r) {  // fonction surchagé dans web_modify_view
    ...
    load_list: function(data) {
        ...
        //Ajoute le multiselect à la bare de recherche
        if (this.$el.find(".search_bar_select").length > 0 ){
            this.$el.find(".search_bar_select").multipleSelect({
            selectAll: false,
            });
        }
    ...
    render_sidebar: function($node) {
        if (!this.sidebar && this.options.sidebar) {
            ...
            var sec_rule = this.getParent().action.security_rule;
            var can_export = (true === sec_rule) || (_.isObject(sec_rule) && sec_rule.can_export && sec_rule.show_export);
            this.sidebar.add_items('other', _.compact([
                can_export && { label: _t("Export"), callback: this.on_sidebar_export },
            ...
            this.$pager
                .on('click', 'a[data-pager-action]', function () {
                    ...
                    .click(function (e) {
                        ...
                        var sec_rule;
                        var obj = self;
                        // = self.getParent().action.security_rule;
                        while (obj.getParent()) {
                            if (obj.getParent().action) {
                                sec_rule = obj.getParent().action.security_rule; break;
                            } else {
                                obj = obj.getParent();
                            }
                        }
                        var is_admin = (true === sec_rule) || (_.isObject(sec_rule) && sec_rule.is_admin);
                        var $select = $('<select>')
                            .appendTo($this.empty())
                            ...
                            .append('<option value="80">80</option>' +
                                    ...
                                    (is_admin ? '<option value="2000">2000</option><option value="NaN">' + _t("Unlimited") + '</option>' : ''))
                            .change(function () {
    ...
    sort_by_column: function (e) {
        ...
        // test whether the field is sortable, si il ne l'est pas on quite le tri
        if (field && !field.sortable) {
            return false;
        }
        // on tri par ordre décroissant au clique
    ...
    select_record:function (index, view) {
        view = view || index === null || index === undefined ? 'form' : ('qualitychart' in this.getParent().views) ? 'qualitychart' : 'form';
     ...
     */
    do_activate_record: function (index, id, dataset, view) { // surchargé dans web_modify_view
        this.dataset.ids = dataset.ids;
    ...
    facet_for: function (field, field_name, val, op, label) {
        return {
            category: field.string,
            field: {
                get_context: function () { return undefined; },
                get_groupby: function () { return []; },
                get_domain: function () { return [[field_name, op, val]]; }
            },
            values: [{label: label || val, value: null}]
        };
    },
    change_list_filters: function(ev) {
        ...
        if (ev.target.offsetParent.className.match(/ms-drop/)){
            return null;
        }
