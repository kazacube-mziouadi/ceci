...
var FieldMany2One = common.AbstractField.extend(common.CompletionFieldMixin, common.ReinitializeFieldMixin, {
    ...
    initialize_field: function() {
        ...
        core.bus.on('click', this, function() {
            if (!this.get("effective_readonly") && this.$input.is(':visible') && this.$input.autocomplete('widget').is(':visible')) {
    ...
    display_string: function(str) {
        ...
        if (!this.get("effective_readonly")) {
            ...
            var $link = this.$el.find('.oe_form_uri')
                 .unbind('mousedown').unbind('click')
                 .html(link);
            if (! this.options.no_open){
                this.$el.find('.oe_form_uri').on('mousedown' ,function(e){
                    if(e.which === 2) {
                        // middle button click, open in new tab
                        var context = self.build_context().eval();
                        var model_obj = new Model(self.field.relation);
                        model_obj.call('get_formview_action', [self.get("value"), context]).then(function(action){
                            window.open('/web#id=' + action.res_id + '&view_type=form&model=' + action.res_model);
                            return false;
                        });
                        return false;
                    }
                ...
                $link.click(function (ev) {
                    ...
                    model_obj.call('get_formview_action', [self.get("value"), context]).then(function(action){
                        if (ev.which == 2) {              
                            // middle button click, open in new tab
                            // ne marche plus avec les naviguateurs récents ?
                            // voire ci-dessous
                            return null;
                            //window.open('/web#id=' + action.res_id + '&view_type=form&model=' + action.res_model);
                        } else {
    ...
    */
    ...
    get_value: function() {
        ...
        _.each(starting_ids, function(id) {
        	if (id.toString().match(data.BufferedDataSet.virtual_id_regex)) {
        		return;
        	}
...
var One2ManyListView = X2ManyListView.extend({
    ...
    do_button_action: function (name, id, callback) {
        ...
        this.save_edition().then(function () {
            ...
            if (!self.x2m.options.reload_on_button && !changed_records) {
                var promise = self.handle_button(name, id, callback);
    ...
    setup_columns: function(fields, grouped) {
        var self = this;
        var ret = this._super(fields, grouped);
        _.each(this.columns, function(col) {
            if (col.column_invisible) {
                var fm = self.getParent().getParent()._ic_field_manager;
                var set_col_invisible = function(){
                    var invisible = fm.compute_domain(py.eval(col.column_invisible));
                    var index = self.$('[data-id=' + col.name + ']').index() + 1;
                    if (invisible) {
                        self.$('td:nth-child(' + index + '),th:nth-child(' + index + ')').hide();
                    } else {
                        self.$('td:nth-child(' + index + '),th:nth-child(' + index + ')').show();
                    }
                }
                self.on('view_list_rendered', self, set_col_invisible);
                fm.on('view_content_has_changed', fm, set_col_invisible);
            }
        })
        return ret;
    },
...
var One2ManyFormView = FormView.extend({
    ...
    init: function(parent, dataset, view_id, options) {
        options.action_buttons = false;
        var self = this;
        this._super(parent, dataset, view_id, options);
        parent.on('switch_mode:list', this, function(){
            self.save();
        });
        this.dataset.x2m.on('load_record', this, function () {
            this.reload();
        ...
        core.bus.on('act_next', this, this.on_act_next);
    },
    on_act_next: function(model){
        if (this.model != model) {
            return;
        }
        this.execute_pager_action('next');
        parent = this.getParent();
        while (parent.reload == undefined) {
    ...
    load_form: function(data) {
        this._super(data);
        var self = this;
        this.$buttons && this.$buttons.find('button.oe_form_button_create').click(function() {
    ...
    destroy: function() {
        core.bus.off('act_next', this, this.on_act_next);
        this.save();
    }
});
