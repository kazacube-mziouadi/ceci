...
var CompletionFieldMixin = {
    ...
    get_search_result: function(search_val) {
        ...
        return this.orderer.add(dataset.name_search(
            ...
            if (search_val.length > 0 && !_.include(raw_result, search_val) &&
                ! (self.options && (self.options.no_create || !self.options.force_quick_create))) {
    ...
    /*build_context: function() {
        return {from_select_create: true}
    },*/
...
var ViewDialog = Dialog.extend({ // FIXME should use ViewManager
    ...
    init: function(parent, options) {
        ...
        options.parent_view && options.parent_view.on('load_record', this, function(){
            this.view_form && this.view_form.reload();
        })
...
var SelectCreateDialog = ViewDialog.extend({
    ...
    sidebar_eval_context: function () {
        return $.when(new data.CompoundContext({select_create:true}));
    },
    ...
    setup: function(search_defaults) {
        ...
        var $header = $('<div/>').addClass('o_modal_header oe-control-panel').appendTo(this.$el);
        var $pager = $('<div/>').addClass('o_pager').appendTo($header);
        var $buttons = $('<div/>').addClass('oe-search-options').appendTo($header);
        ...
        this.searchview.prependTo($header).done(function() {
            ...
            self.view_list.appendTo(self.$el).then(function() {
                ...
                self.view_list.sidebar = new Sidebar(self, {});
                self.view_list.sidebar.add_toolbar(self.view_list.fields_view.toolbar);
                self.view_list.sidebar.appendTo($buttons);
