...
var WebClient = Widget.extend({
    events: {
        ...
        'click #btn_home': 'on_home_click',
        'click .btn_home_menu': 'on_home_menu_click',
        'click .home_selected a': 'on_link_home',
        'click .edit_page': 'on_edit_page',
    },
    custom_events: {
    ...
    init: function(parent, client_options) {
        ...
        this.set('title_part', {"zopenerp": "Open-prod"});
    },
    start: function() {
        var self = this;
    ...
    show_common: function() {
        ...
        if (!session.registered_contract) {
        	self.$el.append('<h3 style="color:red; position:absolute; bottom: 0;text-align:center;width:100%">'+_t('Demonstration version, not suitable for production')+'</h1>');
        };
    ...
    */
    ...
    on_home_click: function() {
        var self = this;
        this.clear_uncommitted_changes().then(function(){
            self.$el.find('.oe_webclient').hide();
            self.$el.find('#homepage').show();
            self.$el.find('#favorites').hide();
        });
    },
    on_home_menu_click: function(ev) {
        var menu_id = ev.currentTarget.dataset.menu;
        if (menu_id) {
            var target_menu = $("a[data-menu="+menu_id+"]");
            if (target_menu.length && target_menu.data('actionModel') == 'ir.actions.act_url') {
                this.do_action(target_menu.data('actionId'));
            }
            this.$el.find('.oe_webclient').show();
            this.$el.find('#homepage').hide();
            this.$el.find('#favorites').hide();
            this.menu.on_change_top_menu(menu_id, false);
        }
    },
    on_link_home: function(ev) {
        if (ev.target.href == window.location.href) {
            $('.home_selected').removeClass('home_selected').hide();
            $('.oe_application').show();
        }
    },
    on_edit_page: function(ev) {
        var page_id = parseInt(ev.target.dataset.pageId);
        var html_to_edit = ev.target.previousElementSibling.outerHTML;
    	var child = $('<div class="modal" style="display:none">\
    	                    <div class="modal-dialog modal-lg">\
    						  <iframe src="/web_openprod/static/editor/index.html" class="modal-content" style="width: 100%;height:100%" />\
    						</div>\
    					</div>').prependTo('body');
    	var iframe = child.find('iframe');
    	iframe.on('load', function(){
    	    $(iframe[0].contentDocument.body).find('#editor').html(html_to_edit);
    	    var editorValidation = $.Deferred();
    	    editorValidation.then(function(res){
    	           ev.target.previousElementSibling.outerHTML = res;
    	           (new Model('web.page')).call('write', [page_id, {inner_content: res}]);
    	           child.modal('hide');
    	           child.remove();
    	        }, function() {
    	           child.modal('hide');
            ...
            iframe[0].contentWindow.editorValidation = editorValidation;
            iframe[0].contentWindow.Model = Model;
            iframe[0].contentWindow._t = core._t;
            iframe[0].contentWindow.session = session;
            iframe[0].contentWindow.page_id = page_id;
    	    var $if = iframe[0].contentWindow.jQuery;
    	    $if('#editor').gridEditor({
                new_row_layouts : [ [ 12 ], [ 6, 6 ], [ 9, 3 ] ],
                content_types: ['trumbowyg'],
                trumbowyg: {
                    csrf_token: core.csrf_token
                }
            });
    	    $if('#editor').on('dragstart', 'img', false);
    	});
    	child.modal();
