...
var View = Widget.extend({
    ...
    load_view: function() {
        ...
        if (this.embedded_view) {
            ...
            if (this.view_type === 'form'){   // ajout de l'active_id pour les vues forms dans le context pour le fields_view_get
                var new_context = this.dataset.get_context();
                if ( this.dataset.context.params) {
                    if ( this.dataset.context.params.id === undefined ){
                        new_context.add({'active_id' : this.dataset.ids[this.dataset.index]});
                    }
                    else {
                        new_context.add({'active_id' : this.dataset.context.params.id });
                    }
                }
                else {
                    new_context.add({'active_id' : this.dataset.ids[this.dataset.index]});
