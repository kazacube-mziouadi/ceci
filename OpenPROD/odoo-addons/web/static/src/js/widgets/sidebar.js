...
var Sidebar = Widget.extend({
    ...
    add_toolbar: function(toolbar) {
        ...
        _.each(['print','action','relate'], function(type) {
            ...
            if (items) {
                ...
                var sec_rule = self.getParent().getParent().action && self.getParent().getParent().action.security_rule;
                var hide_reports = (sec_rule === false) || (_.isObject(sec_rule) && sec_rule.hide_reports);
                if (type=='print' && !hide_reports){
                    self.add_items('print', items);
                } else {
    ...
    on_item_action_clicked: function(item) {
        ...
        self.getParent().sidebar_eval_context().done(function (sidebar_eval_context) {
            ...
            if (ids.length === 0 && !(item.action.context && py.eval(item.action.context).sidebar_without_id)) {
            ...
            var active_ids_context = {
                active_id: (ids.length && ids[0]) || null,
                active_ids: ids,
            ...
            $.when(domain).done(function (domain) {
                ...
                self.rpc("/web/action/load", {
                    ...
                    self.do_action(result, {
                        on_close: function() {
                            // reload view
                            if (self.getParent().reload){
                                self.getParent().reload();
                            } else {
                                Array.prototype.push.apply(self.getParent().view_list.groups.datagroup.model._domain, window.new_domain);
                                delete window.new_domain;
                                self.getParent().view_list.reload();
                            }
                        },
                        additional_context: {select_create:!!c.select_create},
                    });
                });
            });
        });
